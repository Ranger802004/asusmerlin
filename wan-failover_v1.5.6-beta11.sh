  ...
  if [ ! -z "$PIDS" ] >/dev/null;then
    ...
    until [ -z "${PIDS}" ] >/dev/null;do
      for PID in ${PIDS};do
        ...
        kill -9 ${PID} \
        || { logger -p 2 -st "${0##*/}" "Restart - ***Error*** Unable to kill ${0##*/} Process ID: "${PID}"" && continue ;} \
        && { logger -p 1 -st "${0##*/}" "Restart - Killed ${0##*/} Process ID: "${PID}"" && PIDS="$(ps | grep -w "$0" | grep -w "run\|manual" | awk '{print $1}')" ;}
      done
    done
    # Execute Cleanup
    . $CONFIGFILE
...
for WANPREFIX in ${WANPREFIXES};do
  ...
  if [[ "$STATUS" == "CONNECTED" ]] || { [[ "$(nvram get ${WANPREFIX}_state_t)" == "2" ]] || [[ "$(nvram get ${WANPREFIX}_auxstate_t)" == "0" ]] ;} >/dev/null;then
...
&& { { [[ "$(nvram get wan1_state_t)" != "2" ]] || [[ "$(nvram get wan1_auxstate_t)" != "0" ]] ;} || { [[ "$(nvram get wan1_gateway)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan1_gw_ifname)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $5}')" ]] ;} ;} >/dev/null;do
  ...
  elif { [[ "$WAN0PACKETLOSS" == "0%" ]] || [[ "$(nvram get wan0_state_t)" == "2" ]] ;} && { [[ "$WAN1PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] || [[ "$(nvram get wan1_auxstate_t)" != "0" ]] ;} >/dev/null;then
    WAN0STATUS=CONNECTED
    WAN1STATUS=DISCONNECTED
    logger -p 6 -t "${0##*/}" "Debug - WAN0: $WAN0STATUS"
    logger -p 6 -t "${0##*/}" "Debug - WAN1: $WAN1STATUS"
