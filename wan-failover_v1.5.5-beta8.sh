...
for FWVERSION in ${FWVERSIONS};do
  if [[ "$(nvram get 3rd-party)" == "merlin" ]] && [[ "$(nvram get buildno)" == "$FWVERSION" ]] >/dev/null;then
    ...
    logger -st "${0##*/}" "Firmware: ***"$(nvram get buildno)" is not supported, issues may occur from running this version***"
  fi
done
setvariables
}
...
if [[ "$VERSION" != "$REMOTEVERSION" ]] >/dev/null;then
  ...
  logger -st "${0##*/}" "Update - Script has been updated ${0##*/}"
elif [[ "$VERSION" == "$REMOTEVERSION" ]] >/dev/null;then
  logger -st "${0##*/}" "Script is up to date - Version: "$VERSION""
fi
}
...
if [[ "${mode}" == "cron" ]] || [[ "${mode}" == "install" ]] || [[ "${mode}" == "restart" ]] || [[ "${mode}" == "update" ]] || [[ "${mode}" == "config" ]] >/dev/null;then
  if [ -z "$(cru l | grep -e "$0")" ] >/dev/null;then
    logger -st "${0##*/}" "Cron - Creating Cron Job"
    cru a setup_wan_failover_run "*/1 * * * *" $0 run
    logger -st "${0##*/}" "Cron - Created Cron Job"
  fi
# Remove Cron Job
elif [[ "${mode}" == "kill" ]] || [[ "${mode}" == "uninstall" ]] >/dev/null;then
  if [ ! -z "$(cru l | grep -e "$0")" ] >/dev/null; then
    logger -st "${0##*/}" "Cron - Removing Cron Job"
    cru d setup_wan_failover_run
    logger -st "${0##*/}" "Cron - Removed Cron Job"
  fi
fi
exit
}
...
if [ ! -z "$BOOTDELAYTIMER" ] >/dev/null;then
  if [[ "$(awk -F "." '{print $1}' "/proc/uptime")" -lt "$BOOTDELAYTIMER" ]] >/dev/null;then
    logger -st "${0##*/}" "Boot Delay - Waiting for System Uptime to reach $BOOTDELAYTIMER seconds"
    while [[ "$(awk -F "." '{print $1}' "/proc/uptime")" -lt "$BOOTDELAYTIMER" ]] >/dev/null;do
      sleep 1
    done
    logger -st "${0##*/}" "Boot Delay - System Uptime is $(awk -F "." '{print $1}' "/proc/uptime") seconds"
  fi
fi
...
# Check if WAN Interfaces are Enabled and Connected
  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    if [[ "$(nvram get "${WANPREFIX}"_enable)" == "0" ]] >/dev/null;then
      logger -st "${0##*/}" "WAN Status - ${WANPREFIX} disabled"
      STATUS=DISABLED
    # Check if WAN is Enabled
      ...
      if [[ "$(nvram get "${WANPREFIX}"_state_t)" == "3" ]] || [[ "$(nvram get "${WANPREFIX}"_state_t)" == "4" ]] >/dev/null;then
        if [[ "$(nvram get "${WANPREFIX}"_state_t)" == "3" ]] >/dev/null;then
          logger -st "${0##*/}" "WAN Status - "${WANPREFIX}": Cable Unplugged"
        elif [[ "$(nvram get "${WANPREFIX}"_state_t)" == "4" ]] >/dev/null;then
          logger -st "${0##*/}" "WAN Status - "${WANPREFIX}": Disconnected"
        fi
        logger -st "${0##*/}" "WAN Status - Restarting "${WANPREFIX}""
        ...
        if [[ "$(nvram get "${WANPREFIX}"_state_t)" == "3" ]] || [[ "$(nvram get "${WANPREFIX}"_state_t)" == "4" ]] >/dev/null;then
          ...
          logger -st "${0##*/}" "WAN Status - Restarted "${WANPREFIX}""
          break
        else
          wanstatus
        fi
      ...
      if { [[ "$(nvram get ${WANPREFIX}_ipaddr)" == "0.0.0.0" ]] || [[ "$(nvram get ${WANPREFIX}_gateway)" == "0.0.0.0" ]] ;} >/dev/null;then
        logger -st "${0##*/}" "WAN Status - ${WANPREFIX} is disconnected.  IP Address: "$(nvram get ${WANPREFIX}_ipaddr)" Gateway: "$(nvram get ${WANPREFIX}_gateway)""
        STATUS=DISCONNECTED
        continue
      fi
      ...
      if [[ "$PACKETLOSS" == "0%" ]] >/dev/null;then
        ...
        logger -st "${0##*/}" "WAN Status - ${WANPREFIX} has $PACKETLOSS packet loss ***Verify $TARGET is a valid server for ICMP Echo Requests***"
        STATUS="DISCONNECTED"
      else
        logger -st "${0##*/}" "WAN Status - ${WANPREFIX} has $PACKETLOSS packet loss"
        STATUS="DISCONNECTED"
      fi
    fi
    # Set WAN Status
...
if [[ "$WAN0STATUS" == "CONNECTED" ]] >/dev/null;then
  logger -st "${0##*/}" "Load Balance Monitor - Monitoring "$WAN0" via "$WAN0TARGET" for Packet Loss"
elif [[ "$WAN0STATUS" != "CONNECTED" ]] >/dev/null;then
  logger -st "${0##*/}" "Load Balance Monitor - Monitoring "$WAN0" via "$WAN0TARGET" for Restoration"
fi
if [[ "$WAN1STATUS" == "CONNECTED" ]] >/dev/null;then
  logger -st "${0##*/}" "Load Balance Monitor - Monitoring "$WAN1" via "$WAN1TARGET" for Packet Loss"
elif [[ "$WAN1STATUS" != "CONNECTED" ]] >/dev/null;then
  logger -st "${0##*/}" "Load Balance Monitor - Monitoring "$WAN1" via "$WAN1TARGET" for Restoration"
fi
...
|| { [[ "$(nvram get wan0_state_t)" != "2" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] ;} >/dev/null;do
  ...
  || { [ -z "$(ip rule list from all iif lo to "$WAN1TARGET" lookup "$WAN1ROUTETABLE")" ] && [[ "$(nvram get wan1_state_t)" == "2" ]] ;} && { [[ "$(nvram get wan1_ipaddr)" != "0.0.0.0" ]] || [[ "$(nvram get wan1_gateway)" != "0.0.0.0" ]] ;} >/dev/null;then
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -st "${0##*/}" "Load Balance Monitor - WAN0 Packet Loss: $WAN0PACKETLOSS WAN1 Packet Loss: $WAN1PACKETLOSS"
    ...
    if [ -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -st "${0##*/}" "Load Balance Monitor - Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS WAN1 Packet Loss: $WAN1PACKETLOSS"
  ...
  && { [[ "$WAN1PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] ;} >/dev/null;then
    ...
    if [ -z "$PACKETLOSSLOGGING" ] || [[ "$PACKETLOSSLOGGING" == "1" ]] >/dev/null;then
      logger -st "${0##*/}" "Load Balance Monitor - Packet Loss Detected - WAN0 Packet Loss: "$WAN0PACKETLOSS""
      logger -st "${0##*/}" "Load Balance Monitor - Packet Loss Detected - WAN1 Packet Loss: "$WAN1PACKETLOSS""
      continue
...
wan0failovermonitor ()
{
  logger -st "${0##*/}" "WAN0 Failover Monitor - Monitoring "$WAN0" via $WAN0TARGET for Failure"
...
&& { [[ "$(nvram get wan0_gateway)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan0_gw_ifname)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $5}')" ]] ;} >/dev/null;do
  ...
  if [[ "$WAN0PACKETLOSS" == "0%" ]] >/dev/null;then
    ...
    logger -st "${0##*/}" "WAN0 Failover Monitor - Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS"
    switchwan
  elif [[ "$WAN0PACKETLOSS" != "0%" ]] >/dev/null;then
    if [ -z "$PACKETLOSSLOGGING" ] || [[ "$PACKETLOSSLOGGING" == "1" ]] >/dev/null;then
      logger -st "${0##*/}" "WAN0 Failover Monitor - Packet Loss Detected - WAN0 Packet Loss: $WAN0PACKETLOSS"
      continue
    elif [ ! -z "$PACKETLOSSLOGGING" ] && [[ "$PACKETLOSSLOGGING" == "0"]] >/dev/null;then
      continue
    fi
...
wan0failbackmonitor ()
{
  logger -st "${0##*/}" "WAN0 Failback Monitor - Monitoring "$WAN0" via $WAN0TARGET for Failback"
...
&& { [[ "$(nvram get wan0_gateway)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan0_gw_ifname)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $5}')" ]] ;} >/dev/null;do
  ...
  if [[ "$WAN0PACKETLOSS" == "100%" ]] >/dev/null;then
    ...
    logger -st "${0##*/}" "WAN0 Failback Monitor - Connection Detected - WAN0 Packet Loss: $WAN0PACKETLOSS"
    switchwan
  elif [[ "$WAN0PACKETLOSS" != "0%" ]] >/dev/null;then
    if [ -z "$PACKETLOSSLOGGING" ] || [[ "$PACKETLOSSLOGGING" == "1" ]] >/dev/null;then
      logger -st "${0##*/}" "WAN0 Failback Monitor - Packet Loss Detected - WAN0 Packet Loss: $WAN0PACKETLOSS"
      continue
    elif [ ! -z "$PACKETLOSSLOGGING" ] && [[ "$PACKETLOSSLOGGING" == "0"]] >/dev/null;then
      continue
    fi
...
{
if [[ "$(nvram get wans_dualwan | awk '{print $2}')" == "none" ]] >/dev/null;then
  logger -st "${0##*/}" "WAN Failover Disabled - Dual WAN is disabled"
elif [[ "$(nvram get wandog_enable)" != "0" ]] >/dev/null;then
  logger -st "${0##*/}" "WAN Failover Disabled - ASUS Factory WAN Failover is enabled"
elif [[ "$(nvram get wan0_enable)" == "0" ]] && [[ "$(nvram get wan1_enable)" == "0" ]] >/dev/null;then
  logger -st "${0##*/}" "WAN Failover Disabled - "$WAN0" and "$WAN1" are disabled"
elif [[ "$(nvram get wan0_enable)" == "0" ]] >/dev/null;then
  logger -st "${0##*/}" "WAN Failover Disabled - "$WAN0" is disabled"
elif [[ "$(nvram get wan1_enable)" == "0" ]] >/dev/null;then
  logger -st "${0##*/}" "WAN Failover Disabled - "$WAN1" is disabled"
fi
  logger -st "${0##*/}" "WAN Failover Disabled - WAN Failover is currently disabled.  ***Review Logs***"
while \
  ...
  && { [[ "$(nvram get wan0_state_t)" == "2" ]] && [[ "$(nvram get wan1_state_t)" == "2" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN0" and "$WAN1" are enabled and connected"
    break
  ...
  && [[ "$(nvram get wan0_state_t)" == "2" ]] && [[ "$(nvram get wan1_primary)" == "1" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN0" is the only enabled WAN interface but is not Primary WAN"
    switchwan && break
  ...
  && [[ "$(nvram get wan1_state_t)" == "2" ]] && [[ "$(nvram get wan0_primary)" == "1" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN1" is the only enabled WAN interface but is not Primary WAN"
    switchwan && break
  ...
  && [[ "$(nvram get wan0_state_t)" == "2" ]] && [[ "$(nvram get wan0_primary)" == "0" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN0" is the only connected WAN interface but is not Primary WAN"
    switchwan && break
  ...
  && [[ "$(nvram get wan1_state_t)" == "2" ]] && [[ "$(nvram get wan1_primary)" == "0" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN1" is the only connected WAN interface but is not Primary WAN"
    switchwan && break
  ...
  && { [[ "$(nvram get wan0_realip_state)" == "2" ]] && [[ "$(nvram get wan1_realip_state)" != "2" ]] && [[ "$(nvram get wan0_primary)" == "0" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN1" has a Real IP Address but is not Primary WAN"
    switchwan && break
  ...
  && [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT | grep -e "packet loss" | awk '{print $7}')" == "0%" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN0" and "$WAN1" have 0% packet loss"
    break
  ...
  && { [[ "$(nvram get wan1_primary)" == "1" ]] && [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT | grep -e "packet loss" | awk '{print $7}')" == "100%" ]] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Failover Disabled - "$WAN0" has 0% packet loss but is not Primary WAN"
    switchwan && break
...
logger -st "${0##*/}" "WAN Failover Disabled - Returning to check WAN Status"
wanstatus
}
...
&& { [[ "$(nvram get "$ACTIVEWAN"_ipaddr)" == "$(nvram get wan_ipaddr)" ]] && [[ "$(nvram get "$ACTIVEWAN"_gateway)" == "$(nvram get wan_gateway)" ]] && [[ "$(nvram get "$ACTIVEWAN"_gw_ifname)" == "$(nvram get wan_gw_ifname)" ]] ;} >/dev/null;do
  ...
  if [[ "$(nvram get "$ACTIVEWAN"_primary)" != "1" ]] && [[ "$(nvram get "$INACTIVEWAN"_primary)" != "0" ]] >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - Switching $ACTIVEWAN to Primary WAN"
  ...
  if [[ "$(nvram get "$ACTIVEWAN"_ipaddr)" != "$(nvram get wan_ipaddr)" ]] >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - WAN IP Address: $(nvram get "$ACTIVEWAN"_ipaddr)"
    nvram set wan_ipaddr=$(nvram get "$ACTIVEWAN"_ipaddr)
  fi

  # Change WAN Gateway
  if [[ "$(nvram get "$ACTIVEWAN"_gateway)" != "$(nvram get wan_gateway)" ]] >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - WAN Gateway: $(nvram get "$ACTIVEWAN"_gateway)"
  ...
  if [[ "$(nvram get "$ACTIVEWAN"_gw_ifname)" != "$(nvram get wan_gw_ifname)" ]] >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - WAN Interface: $(nvram get "$ACTIVEWAN"_gw_ifname)"
  ...
  if [[ "$(nvram get "$ACTIVEWAN"_ifname)" != "$(nvram get wan_ifname)" ]] >/dev/null;then
    if [[ "$(nvram get "$ACTIVEWAN"_ifname)" != "$(nvram get "$ACTIVEWAN"_gw_ifname)" ]] >/dev/null;then
      logger -st "${0##*/}" "WAN Switch - WAN Interface: $(nvram get "$ACTIVEWAN"_ifname)"
    fi
...
# Switch DNS
  ...
  if [ ! -z "$(pidof AdGuardHome)" ] || { [ -f "/opt/etc/AdGuardHome/.config" ] && [ ! -z "$(awk -F "=" '/ADGUARD_LOCAL/ {print $2}' "/opt/etc/AdGuardHome/.config" | sed -e 's/^"//' -e 's/"$//' | grep -w ^"YES")" ] ;} >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - DNS is being managed by AdGuard"
  # Change Manual DNS Settings
    ...
    if [ ! -z "$(nvram get "$ACTIVEWAN"_dns1_x)" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get "$ACTIVEWAN"_dns1_x))")" ] >/dev/null;then
      logger -st "${0##*/}" "WAN Switch - DNS1 Server: "$(nvram get "$ACTIVEWAN"_dns1_x)""
      ...
      logger -st "${0##*/}" "WAN Switch - $DNSRESOLVFILE already updated for $ACTIVEWAN DNS1 Server"
    elif [ -z "$(nvram get "$ACTIVEWAN"_dns1_x)" ] >/dev/null;then
      logger -st "${0##*/}" "WAN Switch - No DNS1 Server for $ACTIVEWAN"
    fi
    ...
    if [ ! -z "$(nvram get "$ACTIVEWAN"_dns2_x)" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get "$ACTIVEWAN"_dns2_x))")" ] >/dev/null;then
      logger -st "${0##*/}" "WAN Switch - DNS2 Server: "$(nvram get "$ACTIVEWAN"_dns2_x)""
      ...
      logger -st "${0##*/}" "WAN Switch - $DNSRESOLVFILE already updated for $ACTIVEWAN DNS2 Server"
    elif [ -z "$(nvram get "$ACTIVEWAN"_dns2_x)" ] >/dev/null;then
      logger -st "${0##*/}" "WAN Switch - No DNS2 Server for $ACTIVEWAN"
    fi
  ...
  # Change Automatic ISP DNS Settings
    ...
    if [[ "$(nvram get "$ACTIVEWAN"_dns)" != "$(nvram get wan_dns)" ]] >/dev/null;then
      logger -st "${0##*/}" "WAN Switch - Automatic DNS Settings from ISP: "$(nvram get "$ACTIVEWAN"_dns)""
    ...
    if [ ! -z "$(echo $(nvram get "$ACTIVEWAN"_dns) | awk '{print $1}')" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get "$ACTIVEWAN"_dns) | awk '{print $1}')")" ] >/dev/null;then
      ...
      logger -st "${0##*/}" "WAN Switch - DNS1 Server not detected in Automatic ISP Settings for $ACTIVEWAN"
    fi
    ...
    if [ ! -z "$(echo $(nvram get "$ACTIVEWAN"_dns) | awk '{print $2}')" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get "$ACTIVEWAN"_dns) | awk '{print $2}')")" ] >/dev/null;then
      ...
      logger -st "${0##*/}" "WAN Switch - DNS2 Server not detected in Automatic ISP Settings for $ACTIVEWAN"
    fi
  else
    logger -st "${0##*/}" "WAN Switch - No DNS Settings Detected"
  fi

  # Delete Old Default Route
  if [ ! -z "$(ip route list default | grep -e "$(nvram get "$INACTIVEWAN"_gw_ifname)")" ]  >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - Deleting default route via "$(nvram get "$INACTIVEWAN"_gateway)" dev "$(nvram get "$INACTIVEWAN"_gw_ifname)""
    ip route del default
  fi
  # Add New Default Route
  if [ -z "$(ip route list default | grep -e "$(nvram get "$ACTIVEWAN"_gw_ifname)")" ]  >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - Adding default route via "$(nvram get "$ACTIVEWAN"_gateway)" dev "$(nvram get "$ACTIVEWAN"_gw_ifname)""
  ...
  if [[ "$(nvram get qos_enable)" == "1" ]] >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - QoS is Enabled"
    if [[ ! -z "$(nvram get qos_obw)" ]] && [[ ! -z "$(nvram get qos_obw)" ]] >/dev/null;then
      logger -st "${0##*/}" "WAN Switch - Applying Manual QoS Bandwidth Settings"
      ...
      logger -st "${0##*/}" "WAN Switch - QoS Settings: Download Bandwidth: $(($(nvram get qos_ibw)/1024))Mbps Upload Bandwidth: $(($(nvram get qos_obw)/1024))Mbps"
    fi
  elif [[ "$(nvram get qos_enable)" == "0" ]] >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - QoS is Disabled"
  fi
  sleep 1
done
  if [[ "$(nvram get "$ACTIVEWAN"_primary)" == "1" ]] && [[ "$(nvram get "$INACTIVEWAN"_primary)" == "0" ]] >/dev/null;then
    logger -st "${0##*/}" "WAN Switch - Switched $ACTIVEWAN to Primary WAN"
  fi
restartservices
}
...
for SERVICE in ${SERVICES};do
  logger -st "${0##*/}" "Service Restart - Restarting $SERVICE service"
  service restart_$SERVICE
  logger -st "${0##*/}" "Service Restart - Restarted $SERVICE service"
done
if [[ "${mode}" == "switchwan" ]] >/dev/null;then
  exit
...
if [[ "${mode}" == "email" ]] && [ ! -z "$OPTION" ] >/dev/null;then
  if [[ "$OPTION" == "enable" ]] >/dev/null;then
    SETSENDEMAIL=1
    logger -st "${0##*/}" "Email Notification - Email Notifications Enabled"
  elif [[ "$OPTION" == "disable" ]] >/dev/null;then
    SETSENDEMAIL=0
    logger -st "${0##*/}" "Email Notification - Email Notifications Disabled"
  else
...
# Determine whether to AMTM or AIProtection Email Configuration
  if [ -f "$AMTM_EMAILCONFIG" ] >/dev/null;then
    ...
    if [ -z "$FROM_ADDRESS" ] || [ -z "$TO_NAME" ] || [ -z "$TO_ADDRESS" ] || [ -z "$USERNAME" ] || [ ! -f "$AMTM_EMAIL_DIR/emailpw.enc" ] || [ -z "$SMTP" ] || [ -z "$PORT" ] || [ -z "$PROTOCOL" ] >/dev/null;then
      logger -st "${0##*/}" "Email Notification - AMTM Email Configuration Incomplete"
    else
    ...
    if [ ! -z "$SMTP_SERVER" ] && [ ! -z "$SMTP_PORT" ] && [ ! -z "$MY_NAME" ] && [ ! -z "$MY_EMAIL" ] && [ ! -z "$SMTP_AUTH_USER" ] && [ ! -z "$SMTP_AUTH_PASS" ] >/dev/null;then
      ...
      logger -st "${0##*/}" "Email Notification - AIProtection Alerts Email Configuration Incomplete"
    fi
  fi
    if [ ! -f "$TMPEMAILFILE" ] >/dev/null;then
      logger -st "${0##*/}" "Email Notification - Email Notification Sent"
    elif [ -f "$TMPEMAILFILE" ] >/dev/null;then
      logger -st "${0##*/}" "Email Notification - Email Notification Failed"
      rm "$TMPEMAILFILE"
    fi
    if [[ "$(nvram get wans_mode)" == "lb" ]] >/dev/null;then
      lbmonitor
