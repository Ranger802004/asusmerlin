...
if [[ "${mode}" == "restart" ]] || [[ "${mode}" == "update" ]] || [[ "${mode}" == "config" ]] || [[ "$[mode}" == "email" ]] >/dev/null;then
    ...
    while [[ "$(date "+%S")" -lt "40" ]] || [[ "$(date "+%S")" -gt "45" ]] >/dev/null;do
      if tty >/dev/null 2>&1;then
        ...
        if [[ "$WAITTIMER" -lt "30" ]] >/dev/null;then
          printf '%b\r' ""${BLUE}"***Waiting to kill ${0##*/}*** Current Wait Time: "${GREEN}""$WAITTIMER" Seconds"${NOCOLOR}""
        elif [[ "$WAITTIMER" -lt "60" ]] >/dev/null;then
          printf '%b\r' ""${BLUE}"***Waiting to kill ${0##*/}*** Current Wait Time: "${YELLOW}""$WAITTIMER" Seconds"${NOCOLOR}""
        elif [[ "$WAITTIMER" -ge "60" ]] >/dev/null;then
          printf '%b\r' ""${BLUE}"***Waiting to kill ${0##*/}*** Current Wait Time: "${RED}""$WAITTIMER" Seconds"${NOCOLOR}""
        fi
      fi
      sleep 1
    done
    ...
    if [ ! -z "$PIDS" ] >/dev/null;then
      ...
      if tty >/dev/null 2>&1;then
        ...
        if [[ "$TIMEOUTTIMER" -ge "60" ]] >/dev/null;then
          printf '%b\r' ""${BLUE}"***Waiting for ${0##*/} to restart from CronJob*** Timeout: "${GREEN}""$TIMEOUTTIMER" Seconds"${NOCOLOR}""
        elif [[ "$TIMEOUTTIMER" -ge "30" ]] >/dev/null;then
          printf '%b\r' ""${BLUE}"***Waiting for ${0##*/} to restart from CronJob*** Timeout: "${YELLOW}""$TIMEOUTTIMER" Seconds"${NOCOLOR}""
        elif [[ "$TIMEOUTTIMER" -ge "0" ]] >/dev/null;then
          printf '%b\r' ""${BLUE}"***Waiting for ${0##*/} to restart from CronJob*** Timeout: "${RED}""$TIMEOUTTIMER" Seconds"${NOCOLOR}""
        fi
      fi
      sleep 1
    fi
  ...
  if [ ! -z "$PIDS" ] >/dev/null;then
    ...
    if tty >/dev/null 2>&1;then
      printf '\033[K'
      printf '%b\r' ""${GREEN}"Successfully Restarted ${0##*/} Process ID(s): $PIDS "${NOCOLOR}""
      sleep 3
      printf '\033[K'
    fi
  elif [ -z "$PIDS" ] >/dev/null;then
