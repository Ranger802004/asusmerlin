...
while \
  ...
  || { [[ "$(nvram get wans_dualwan | awk '{print $2}')" == "usb" ]] && [[ "$(nvram get wan1_state_t)" == "0" ]] && [[ "$(nvram get wan1_is_usb_modem_ready)" == "1" ]] ;} >/dev/null;then
    ...
    i=0
    break
  # Restart WAN0 if USB Device is not in Ready State
  elif { [[ "$(nvram get wans_dualwan | awk '{print $1}')" == "usb" ]] && [[ "$(nvram get wan0_state_t)" == "0" ]] && [[ "$(nvram get wan0_is_usb_modem_ready)" == "0" ]] ;} >/dev/null;then
    if [[ "$i" == "1" ]] >/dev/null;then
      logger -p 2 -st "${0##*/}" "WAN Failover Disabled - "$WAN0" USB Device Unplugged, restarting "$WAN0" Interface"
    fi
    i=$(($i+1))
    service "restart_wan_if 0" &
  # Restart WAN1 if USB Device is not in Ready State
  elif { [[ "$(nvram get wans_dualwan | awk '{print $2}')" == "usb" ]] && [[ "$(nvram get wan1_state_t)" == "0" ]] && [[ "$(nvram get wan1_is_usb_modem_ready)" == "0" ]] ;} >/dev/null;then
    if [[ "$i" == "1" ]] >/dev/null;then
      logger -p 2 -st "${0##*/}" "WAN Failover Disabled - "$WAN1" USB Device Unplugged, restarting "$WAN1" Interface"
    fi
    i=$(($i+1))
...
if [ ! -z "$(cru l | grep -w "YazFi")" ] && [ -f "/jffs/scripts/YazFi" ] >/dev/null;then
  logger -p 4 -st "${0##*/}" "Service Restart - Executing YazFi Check"
...
if [[ "$(awk -F "." '{print $1}' "/proc/uptime")" -ge "$(($SKIPEMAILSYSTEMUPTIME+$BOOTDELAYTIMER))" ]] && { [ -f "$AIPROTECTION_EMAILCONFIG" ] || [ -f "$AMTM_EMAILCONFIG" ] ;} >/dev/null;then
  ...
  if [[ "$(nvram ddns_enable_x)" == "1" ]] && [ ! -z "$(nvram get ddns_hostname_x)" ] >/dev/null;then
