...
for WANPREFIX in ${WANPREFIXES};do
  ...
  if [ ! -z "$(ip rule list from all to "$TARGET" lookup "$TABLE")" ] >/dev/null;then
    ...
    until [ -z "$(ip rule list from all to "$TARGET" lookup "$TABLE")" ] >/dev/null;do
      ...
      || { logger -p 2 -t "${0##*/}" "Cleanup - ***Error*** Unable to delete IP Rule for "$TARGET" to monitor "${WANPREFIX}"" && continue ;}
  ...
  if [ -f "/tmp/${WANPREFIX}packetloss.tmp" ] >/dev/null;then
    ...
    || { logger -p 2 -t "${0##*/}" "Cleanup - ***Error*** Unable to delete /tmp/"${WANPREFIX}"packetloss.tmp" && continue ;}
...
if [ -f "$LOCKFILE" ] >/dev/null;then
  ...
  rm -f "$LOCKFILE" \
  || { logger -p 2 -t "${0##*/}" "Cleanup - ***Error*** Unable to delete "$LOCKFILE"" && continue ;}
...
if [[ "${mode}" == "restart" ]] || [[ "${mode}" == "update" ]] || [[ "${mode}" == "config" ]] || [[ "$[mode}" == "email" ]] >/dev/null;then
  ...
  if [ ! -z "$PIDS" ] >/dev/null;then
    ...
    for PID in ${PIDS};do
      ...
      kill -9 ${PID} || { logger -p 2 -st "${0##*/}" "Restart - ***Error*** Unable to kill ${0##*/} Process ID: "${PID}"" && continue ;}
...
if [[ "$WAN0STATUS" == "DISABLED" ]] && [[ "$WAN1STATUS" == "DISABLED" ]] >/dev/null;then
  ...
  logger -p 5 -t "${0##*/}" "WAN Status - Verifying WAN0"
  ...
  logger -p 5 -t "${0##*/}" "WAN Status - Verifying WAN1"
...
for WANPREFIX in ${WANPREFIXES};do
  ...
  if [[ "$(nvram get wans_mode)" == "lb" ]] >/dev/null;then
    ...
    if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /balance/ && /'$(nvram get lan_ifname)'/ && /state/ && /NEW/ ) print}')" ] >/dev/null;then
      ...
      logger -p 4 -t "${0##*/}" "Check IP Rules - Added IPTables MANGLE Balance Rule"
    fi
...
pingwan0target ()
{
ping -I $(nvram get wan0_gw_ifname) $WAN0TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}' > /tmp/wan0packetloss.tmp
return
}
...
pingwan1target ()
{
ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}' > /tmp/wan1packetloss.tmp
return
}
...
while [ "$i" -le "$RECURSIVEPINGCHECK" ] >/dev/null;do
  ...
  if [[ "$WAN0PACKETLOSS" == "0%" ]] && [[ "$WAN1PACKETLOSS" == "0%" ]] >/dev/null;then
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] >/dev/null;then
      logger -p 1 -st "${0##*/}" "Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS"
    fi
    if tty >/dev/null 2>&1;then
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      logger -p 1 -st "${0##*/}" "Failure Detected - WAN1 Packet Loss: $WAN1PACKETLOSS"
    fi
    if tty >/dev/null 2>&1;then
    ...
    if [[ "$(nvram get wans_mode)" == "lb" ]] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      logger -p 1 -st "${0##*/}" "Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS"
      logger -p 1 -st "${0##*/}" "Failure Detected - WAN1 Packet Loss: $WAN1PACKETLOSS"
    elif [[ "$(nvram get wans_mode)" != "lb" ]] && { [[ "$(nvram get wan0_primary)" == "1" ]] || [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] ;} >/dev/null;then
      logger -p 1 -st "${0##*/}" "Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS"
    elif [[ "$(nvram get wans_mode)" != "lb" ]] && { [[ "$(nvram get wan1_primary)" == "1" ]] || [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] ;} >/dev/null;then
      logger -p 1 -st "${0##*/}" "Failure Detected - WAN1 Packet Loss: $WAN1PACKETLOSS"
    fi
    if tty >/dev/null 2>&1;then
...
&& { { [[ "$(nvram get wan0_state_t)" != "2" ]] || [[ "$(nvram get wan0_auxstate_t)" != "0" ]] ;} || { [[ "$(nvram get wan0_gateway)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan0_gw_ifname)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $5}')" ]] ;} ;} \
&& { { [[ "$(nvram get wan1_state_t)" != "2" ]] || [[ "$(nvram get wan1_auxstate_t)" != "0" ]] ;} || { [[ "$(nvram get wan1_gateway)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan1_gw_ifname)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $5}')" ]] ;} ;} >/dev/null;do
  ...
  || { { [ -z "$(ip rule list from all iif lo to "$WAN1TARGET" lookup "$WAN1ROUTETABLE")" ] && [[ "$(nvram get wan1_state_t)" == "2" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} && { [[ "$(nvram get wan1_ipaddr)" != "0.0.0.0" ]] || [[ "$(nvram get wan1_gateway)" != "0.0.0.0" ]] ;} ;} >/dev/null;then
    logger -p 6 -t "${0##*/}" "Debug - WAN Target IP Rule Missing"
    break
  elif [[ "$WAN0PACKETLOSS" == "0%" ]] && [[ "$WAN1PACKETLOSS" == "0%" ]] >/dev/null;then
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -p 4 -st "${0##*/}" "Load Balance Monitor - Adding nexthop via $(nvram get wan0_gateway) dev $(nvram get wan0_gw_ifname) weight $(nvram get wans_lb_ratio | awk -F ":" '{print $1}')"
      logger -p 4 -st "${0##*/}" "Load Balance Monitor - Adding nexthop via $(nvram get wan1_gateway) dev $(nvram get wan1_gw_ifname) weight $(nvram get wans_lb_ratio | awk -F ":" '{print $2}')"
    ...
    if [ -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -p 4 -st "${0##*/}" "Load Balance Monitor - Removing nexthop via $(nvram get wan0_gateway) dev $(nvram get wan0_gw_ifname) weight $(nvram get wans_lb_ratio | awk -F ":" '{print $1}')"
      logger -p 6 -t "${0##*/}" "Debug - Deleting Default Route"
      ip route del default
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -p 4 -st "${0##*/}" "Load Balance Monitor - Removing nexthop via $(nvram get wan1_gateway) dev $(nvram get wan1_gw_ifname) weight $(nvram get wans_lb_ratio | awk -F ":" '{print $2}')"
      logger -p 6 -t "${0##*/}" "Debug - Deleting Default Route"
      ip route del default
...
&& { { [[ "$(nvram get wan1_state_t)" != "2" ]] || [[ "$(nvram get wan1_auxstate_t)" != "0" ]] ;} || { [[ "$(nvram get wan1_gateway)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan1_gw_ifname)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $5}')" ]] ;} ;} >/dev/null;do
  ...
  || { { [ -z "$(ip rule list from all iif lo to "$WAN1TARGET" lookup "$WAN1ROUTETABLE")" ] && [[ "$(nvram get wan1_state_t)" == "2" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} && { [[ "$(nvram get wan1_ipaddr)" != "0.0.0.0" ]] || [[ "$(nvram get wan1_gateway)" != "0.0.0.0" ]] ;} ;} >/dev/null;then
    ...
    continue
  elif { [[ "$WAN0PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan0_state_t)" != "2" ]] || [[ "$(nvram get wan0_auxstate_t)" != "0" ]] ;} && { [[ "$WAN1PACKETLOSS" == "0%" ]] || [[ "$(nvram get wan1_state_t)" == "2" ]] || [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} >/dev/null;then
    WAN0STATUS=DISCONNECTED
    WAN1STATUS=CONNECTED
  ...
  elif { [[ "$WAN0PACKETLOSS" == "0%" ]] || [[ "$(nvram get wan0_state_t)" == "2" ]] ;} && { [[ "$WAN1PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] ;} >/dev/null;then
    wanstatus || return && break
  elif { [[ "$WAN0PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan0_state_t)" != "2" ]] || [[ "$(nvram get wan0_auxstate_t)" == "0" ]] ;} && { [[ "$WAN1PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] || [[ "$(nvram get wan1_auxstate_t)" != "0" ]] ;} >/dev/null;then
    wanstatus || return && break
  elif [[ "$WAN0PACKETLOSS" != "0%" ]] || [[ "$WAN1PACKETLOSS" != "0%" ]] >/dev/null;then
    continue
  fi
...
&& { [[ "$(nvram get wan1_state_t)" != "2" ]] || { [[ "$(nvram get wan1_gateway)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan1_gw_ifname)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $5}')" ]] ;} ;} >/dev/null;do
  ...
  || { { [ -z "$(ip rule list from all iif lo to "$WAN1TARGET" lookup "$WAN1ROUTETABLE")" ] && [[ "$(nvram get wan1_state_t)" == "2" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} && { [[ "$(nvram get wan1_ipaddr)" != "0.0.0.0" ]] || [[ "$(nvram get wan1_gateway)" != "0.0.0.0" ]] ;} ;} >/dev/null;then
    logger -p 6 -t "${0##*/}" "Debug - WAN Target IP Rule Missing"
    break
  elif { [[ "$WAN0PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan0_auxstate_t)" != "0" ]] ;} && { [[ "$WAN1PACKETLOSS" == "0%" ]] || [[ "$(nvram get wan1_state_t)" == "2" ]] || [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} >/dev/null;then
    if [[ "$(nvram get wan1_state_t)" != "2" ]] >/dev/null;then
      nvram set wan1_state_t=2
    fi
    continue
  elif [[ "$WAN0PACKETLOSS" == "0%" ]] || { { [[ "$WAN0PACKETLOSS" == "0%" ]] || [[ "$(nvram get wan0_state_t)" == "2" ]] || [[ "$(nvram get wan0_auxstate_t)" == "0" ]] ;} && { [[ "$WAN1PACKETLOSS" == "100%" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] || [[ "$(nvram get wan1_auxstate_t)" != "0" ]] ;} ;} >/dev/null;then
    if [[ "$WAN0PACKETLOSS" == "0%" ]] || [[ "$(nvram get wan0_state_t)" == "2" ]] >/dev/null;then
      WAN0STATUS=CONNECTED
    else
      WAN0STATUS=DISCONNECTED
    fi
    if [[ "$WAN1PACKETLOSS" == "0%" ]] || [[ "$(nvram get wan1_state_t)" == "2" ]] >/dev/null;then
      WAN1STATUS=CONNECTED
    else
      WAN1STATUS=DISCONNECTED
    fi
...
while \
  ...
  # WAN Disabled if an interface is Disabled - Load Balance Mode
    ...
    if [[ "$i" == "1" ]] >/dev/null;then
      logger -p 2 -st "${0##*/}" "WAN Failover Disabled - Load Balance Mode: "$WAN0" or "$WAN1" is not Enabled"
    fi
    i=$(($i+1))
    sleep $WANDISABLEDSLEEPTIMER
    continue
  ...
  && { { [[ "$(nvram get wan0_state_t)" == "2" ]] && [[ "$(nvram get wan0_auxstate_t)" == "0" ]] ;} && { [[ "$(nvram get wan1_state_t)" == "2" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} ;} >/dev/null;then
  ...
  && { { [[ "$(nvram get wan0_realip_state)" == "2" ]] && [[ "$(nvram get wan0_auxstate_t)" == "0" ]] ;} && { [[ "$(nvram get wan1_realip_state)" == "2" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} ;} >/dev/null;then
    logger -p 4 -st "${0##*/}" "WAN Failover Disabled - "$WAN0" and "$WAN1" are enabled and connected"
    i=0
    break
  ...
  && { [[ "$(nvram get wan0_state_t)" == "2" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} && [[ "$(nvram get wan0_primary)" == "0" ]] ;} >/dev/null;then
  ...
  && { [[ "$(nvram get wan0_enable)" == "1" ]] && [[ "$(nvram get wan1_enable)" == "1" ]] \
  && { [[ "$(nvram get wan1_state_t)" == "2" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} && [[ "$(nvram get wan1_primary)" == "0" ]] ;} >/dev/null;then
  ...
  && { [[ "$(nvram get wan0_enable)" == "1" ]] && [[ "$(nvram get wan1_enable)" == "1" ]] ;} \
  && { [[ "$(nvram get wan0_realip_state)" == "2" ]] && [[ "$(nvram get wan1_realip_state)" != "2" ]] && [[ "$(nvram get wan0_primary)" == "0" ]] && [[ "$(nvram get wan0_auxstate_t)" == "0" ]] ;} >/dev/null;then
    logger -p 3 -st "${0##*/}" "WAN Failover Disabled - Failover Mode: "$WAN1" has a Real IP Address but is not Primary WAN"
    i=0
    failover || return && break
    break
  ...
  && { [[ "$(nvram get wan0_enable)" == "1" ]] && [[ "$(nvram get wan1_enable)" == "1" ]] ;} \
  && { [[ "$(nvram get wan0_realip_state)" != "2" ]] && [[ "$(nvram get wan1_realip_state)" == "2" ]] && [[ "$(nvram get wan1_primary)" == "0" ]] && [[ "$(nvram get wan1_auxstate_t)" == "0" ]] ;} >/dev/null;then
    logger -p 3 -st "${0##*/}" "WAN Failover Disabled - Failover Mode: "$WAN1" has a Real IP Address but is not Primary WAN"
    i=0
    failover || return && break
    break
  # Return to WAN Status if WAN0 and WAN1 are pinging both Target IP Addresses.
  elif { [[ "$(ping -I $(nvram get wan0_gw_ifname) $WAN0TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}')" == "0%" ]] \
  && [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}')" == "0%" ]] ;} >/dev/null;then
  ...
  && [[ "$(ping -I $(nvram get wan0_gw_ifname) $WAN0TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}')" == "0%" ]] \
  && { [[ "$(nvram get wan1_primary)" == "1" ]] && [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}')" == "100%" ]] ;} >/dev/null;then
  ...
  && [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}')" == "0%" ]] \
  && { [[ "$(nvram get wan0_primary)" == "1" ]] && [[ "$(ping -I $(nvram get wan0_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | awk '/packet loss/ {print $7}')" == "100%" ]] ;} >/dev/null;then
    logger -p 3 -st "${0##*/}" "WAN Failover Disabled - "$WAN0" has 0% packet loss but is not Primary WAN"
    i=0
    failover || return && break
    break
  # WAN Disabled if WAN0 is not Enabled
    ...
    sleep $WANDISABLEDSLEEPTIMER
    continue
  # WAN Disabled if WAN1 is not Enabled
  ...
  # WAN Failover Disabled if not in Dual WAN Mode Failover Mode or if ASUS Factory Failover is Enabled
    ...
    if [[ "$i" == "1" ]] >/dev/null;then
      logger -p 2 -st "${0##*/}" "WAN Failover Disabled - Dual WAN is not Enabled"
    fi
    i=$(($i+1))
    sleep $WANDISABLEDSLEEPTIMER
    continue
...
&& { [[ "$(nvram get "$ACTIVEWAN"_ipaddr)" == "$(nvram get wan_ipaddr)" ]] && [[ "$(nvram get "$ACTIVEWAN"_gateway)" == "$(nvram get wan_gateway)" ]] && [[ "$(nvram get "$ACTIVEWAN"_gw_ifname)" == "$(nvram get wan_gw_ifname)" ]] ;} >/dev/null;do
  ...
  if [[ "$(nvram get qos_enable)" == "1" ]] >/dev/null;then
    ...
    if [[ ! -z "$(nvram get qos_obw)" ]] && [[ ! -z "$(nvram get qos_obw)" ]] >/dev/null;then
      logger -p 5 -st "${0##*/}" "WAN Switch - Applying Manual QoS Bandwidth Settings"
      ...
      logger -p 4 -st "${0##*/}" "WAN Switch - Applied Manual QoS Bandwidth Settings"
      logger -p 4 -st "${0##*/}" "WAN Switch - QoS Settings: Download Bandwidth: $(($(nvram get qos_ibw)/1024))Mbps Upload Bandwidth: $(($(nvram get qos_obw)/1024))Mbps"
    fi
...
# Reset VPNMON-R2
logger -p 6 -t "${0##*/}" "Debug - Checking if VPNMON-R2 is installed"
if [ ! -z "$(pidof vpnmon-r2.sh)" ] && [ -f "/jffs/scripts/vpnmon-r2.sh" ] && [ -f "/jffs/addons/vpnmon-r2.d" ] >/dev/null;then
...
if [[ "$(awk -F "." '{print $1}' "/proc/uptime")" -ge "$(($SKIPEMAILSYSTEMUPTIME+$BOOTDELAYTIMER))" ]] && { [ -f "$AIPROTECTION_EMAILCONFIG" ] || [ -f "$AMTM_EMAILCONFIG" ] ;} >/dev/null;then
  ...
  if [ -f "$AMTM_EMAILCONFIG" ] >/dev/null;then
	...
	$(/usr/sbin/curl --connect-timeout $EMAILTIMEOUT --max-time $EMAILTIMEOUT --url $PROTOCOL://$SMTP:$PORT \
		...
		&& $(rm "$TMPEMAILFILE" & logger -p 4 -st "${0##*/}" "Email Notification - Email Notification Sent") || $(rm "$TMPEMAILFILE" & logger -p 2 -st "${0##*/}" "Email Notification - Email Notification Failed")
    fi
  elif [ -f "$AIPROTECTION_EMAILCONFIG" ] >/dev/null;then
