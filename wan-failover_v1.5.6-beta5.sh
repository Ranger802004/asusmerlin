  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    # Check if WAN is Enabled
      ...
      if [ -z "$(ip rule list from all iif lo to $TARGET oif "$(nvram get ${WANPREFIX}_gw_ifname)" lookup ${TABLE} priority "$PRIORITY")" ] >/dev/null;then
        ...
        logger -p 5 -t "${0##*/}" "WAN Status - Added IP Rule for "$TARGET" to monitor "${WANPREFIX}""
      fi
...
&& { { { [[ "$(nvram get wan0_gateway)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan0_gw_ifname)" == "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $5}')" ]] ;} \
&& { [[ "$(nvram get wan1_gateway)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $3}')" ]] && [[ "$(nvram get wan1_gw_ifname)" == "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $5}')" ]] ;} \
&& { { [ ! -z "$(ip rule list from all iif lo to "$WAN0TARGET" oif "$(nvram get wan0_gw_ifname)" lookup "$WAN0ROUTETABLE")" ] && [[ "$(nvram get wan0_state_t)" == "2" ]] ;} && { [[ "$(nvram get wan0_ipaddr)" != "0.0.0.0" ]] || [[ "$(nvram get wan0_gateway)" != "0.0.0.0" ]] ;} ;} \
&& { { [ ! -z "$(ip rule list from all iif lo to "$WAN1TARGET" oif "$(nvram get wan1_gw_ifname)" lookup "$WAN1ROUTETABLE")" ] && [[ "$(nvram get wan1_state_t)" == "2" ]] ;} && { [[ "$(nvram get wan1_ipaddr)" != "0.0.0.0" ]] || [[ "$(nvram get wan1_gateway)" != "0.0.0.0" ]] ;} ;} ;} \
|| { [[ "$(nvram get wan0_state_t)" != "2" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] ;} ;} >/dev/null;do
  pingtargets || wanstatus
...
for WANPREFIX in ${WANPREFIXES};do
  ...
  # Set Status for Switch WAN Mode
  if { { [[ "$(nvram get wans_mode)" == "fo" ]] || [[ "$(nvram get wans_mode)" == "fb" ]] ;} && [[ "$(nvram get ${WANPREFIX}_primary)" == "1" ]] ;} >/dev/null;then
    STATUS="CONNECTED"
  elif { { [[ "$(nvram get wans_mode)" == "fo" ]] || [[ "$(nvram get wans_mode)" == "fb" ]] ;} && [[ "$(nvram get ${WANPREFIX}_primary)" == "0" ]] ;} >/dev/null;then
    STATUS="DISCONNECTED"
  ...
  # Check DNS if Status is Connected or Primary WAN
    ...
    if [[ "$(nvram get ${WANPREFIX}_dnsenable_x)" == "0" ]] >/dev/null;then
      ...
      if [ ! -z "$(nvram get ${WANPREFIX}_dns1_x)" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns1_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding ${WANPREFIX} DNS1 Server: "$(nvram get ${WANPREFIX}_dns1_x)""
      ...
      if [ ! -z "$(nvram get ${WANPREFIX}_dns2_x)" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns2_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding ${WANPREFIX} DNS2 Server: "$(nvram get ${WANPREFIX}_dns2_x)""
        if { [[ "$(nvram get wans_mode)" == "fo" ]] || [[ "$(nvram get wans_mode)" == "fb" ]] ;} >/dev/null;then
    ...
    # Change Automatic ISP DNS Settings
      ...
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding ${WANPREFIX} DNS1 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')""
      ...
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding ${WANPREFIX} DNS2 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')""
  ...
  # Check DNS if Status is Disconnected or not Primary WAN
    ...
    if [[ "$(nvram get ${WANPREFIX}_dnsenable_x)" == "0" ]] >/dev/null;then
      ...
      if [ ! -z "$(nvram get ${WANPREFIX}_dns1_x)" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns1_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing ${WANPREFIX} DNS1 Server: "$(nvram get ${WANPREFIX}_dns1_x)""
        sed -i '/nameserver '$(nvram get ${WANPREFIX}_dns1_x)'/d' $DNSRESOLVFILE
      fi
      # Change Manual DNS2 Server
      if [ ! -z "$(nvram get ${WANPREFIX}_dns2_x)" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns2_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing ${WANPREFIX} DNS2 Server: "$(nvram get ${WANPREFIX}_dns2_x)""
    ...
    # Remove Automatic ISP DNS Settings
      ...
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing ${WANPREFIX} DNS1 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')""
        sed -i '/nameserver '$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')'/d' $DNSRESOLVFILE
      fi
      # Remove Automatic DNS2 Server
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing ${WANPREFIX} DNS2 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')""
