...
WAN0="$(echo $WANPREFIXES | awk '{print $1}')"
WAN1="$(echo $WANPREFIXES | awk '{print $2}')"
NOCOLOR="\033[0m"
RED="\033[0;31m"
GREEN="\033[0;32m"
YELLOW="\033[0;33m"
...
# Check if ASUS Factory WAN Failover is Enabled
  ...
  wandisabled
# Check Version of IP
elif [[ "$(ip -V | awk -F "iproute2-" '{print $2}')" != "5.11.0" ]] >/dev/null;then
  logger -t "${0##*/}" "IP Utility is an unsupported version: "$(ip -V | awk -F "-" '{print $2}')""
  wandisabled
  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    # Check if WAN is Enabled
      ...
      if [[ "${WANPREFIX}" == "$WAN0" ]] >/dev/null;then
        ...
        # Check WAN0 IP Rule
        if [ -z "$(ip rule list from all iif lo to $WAN0TARGET lookup ${WANPREFIX} priority 100)" ] >/dev/null;then
          logger -t "${0##*/}" "WAN Status - Adding IP Rule for "$WAN0TARGET""
          ip rule add from all iif lo to $WAN0TARGET table ${WANPREFIX} priority 100
      ...
      # Check WAN1 Connection
        ...
        # Check WAN1 IP Rule
        if [ -z "$(ip rule list from all iif lo to $WAN1TARGET lookup ${WANPREFIX} priority 100)" ] >/dev/null;then
          logger -t "${0##*/}" "WAN Status - Adding IP Rule for "$WAN1TARGET""
          ip rule add from all iif lo to $WAN1TARGET table ${WANPREFIX} priority 100
...
if [[ "$(nvram get wans_mode)" == "lb" ]] >/dev/null;then
  ...
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get lan_ifname)'/ && /connmark match/ && /0x80000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IPTables MANGLE match rule for $(nvram get lan_ifname) marked with 0x80000000"
  ...
  if [ -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get wan0_gw_ifname)'/ && /connmark match/ && /0x80000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IPTables MANGLE match rule for $(nvram get wan0_gw_ifname) marked with 0x80000000"
    iptables -t mangle -A OUTPUT -o $(nvram get wan0_gw_ifname) -m connmark --mark 0x80000000/0x80000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  if [ ! -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get wan0_gw_ifname)'/ && /connmark match/ && /0x90000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Deleting IPTables MANGLE match rule for $(nvram get wan0_gw_ifname) marked with 0x90000000"
    iptables -t mangle -D OUTPUT -o $(nvram get wan0_gw_ifname) -m connmark --mark 0x90000000/0x90000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  ...
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get lan_ifname)'/ && /connmark match/ && /0x90000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IPTables MANGLE match rule for $(nvram get lan_ifname) marked with 0x90000000"
  ...
  if [ -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get wan1_gw_ifname)'/ && /connmark match/ && /0x90000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IPTables MANGLE match rule for $(nvram get wan1_gw_ifname) marked with 0x90000000"
    iptables -t mangle -A OUTPUT -o $(nvram get wan1_gw_ifname) -m connmark --mark 0x90000000/0x90000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  if [ ! -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get wan1_gw_ifname)'/ && /connmark match/ && /0x80000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Deleting IPTables MANGLE match rule for $(nvram get wan1_gw_ifname) marked with 0x80000000"
    iptables -t mangle -D OUTPUT -o $(nvram get wan1_gw_ifname) -m connmark --mark 0x80000000/0x80000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  # Check IPTables Mangle Set XMark Rule for WAN0 for PREROUTING Table
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get wan0_gw_ifname)'/ && /state/ && /NEW/ && /CONNMARK/ && /xset/ && /0x80000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IPTables MANGLE set xmark rule for $(nvram get wan0_gw_ifname)"
    iptables -t mangle -A PREROUTING -i $(nvram get wan0_gw_ifname) -m state --state NEW -j CONNMARK --set-xmark 0x80000000/0xf0000000
  fi
  # Check IPTables Mangle Set XMark Rule for WAN1 for PREROUTING Table
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get wan1_gw_ifname)'/ && /state/ && /NEW/ && /CONNMARK/ && /xset/ && /0x90000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IPTables MANGLE set xmark rule for $(nvram get wan1_gw_ifname)"
    iptables -t mangle -A PREROUTING -i $(nvram get wan1_gw_ifname) -m state --state NEW -j CONNMARK --set-xmark 0x90000000/0xf0000000
  fi
  # Create WAN0 IP Address Rule
  if [ -z "$(ip rule list from $(nvram get wan0_ipaddr) lookup "$WAN0" priority 200)" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get wan0_ipaddr) lookup "$WAN0""
    ip rule add from $(nvram get wan0_ipaddr) lookup "$WAN0" priority 200
  fi
  # Create WAN0 Gateway IP Rule
  if [ -z "$(ip rule list from all to $(nvram get wan0_gateway) lookup "$WAN0" priority 400)" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get wan0_gateway) lookup "$WAN0""
    ip rule add from all to $(nvram get wan0_gateway) lookup "$WAN0" priority 400
  fi
  # Create WAN0 DNS IP Rules
  if [[ "$(nvram get wan0_dnsenable_x)" == "0" ]] >/dev/null;then
    if [ ! -z "$(nvram get wan0_dns1_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get wan0_dns1_x) lookup "$WAN0" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get wan0_dns1_x) lookup "$WAN0""
        ip rule add from $(nvram get wan0_dns1_x) lookup "$WAN0" priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get wan0_dns1_x) lookup "$WAN0" priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get wan0_dns1_x) lookup "$WAN0""
        ip rule add from all to $(nvram get wan0_dns1_x) lookup "$WAN0" priority 400
      fi
    fi
    if [ ! -z "$(nvram get wan0_dns2_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get wan0_dns2_x) lookup "$WAN0" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get wan0_dns2_x) lookup "$WAN0""
        ip rule add from $(nvram get wan0_dns2_x) lookup "$WAN0" priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get wan0_dns2_x) lookup "$WAN0" priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get wan0_dns2_x) lookup "$WAN0""
        ip rule add from all to $(nvram get wan0_dns2_x) lookup "$WAN0" priority 400
      fi
    fi
  elif [[ "$(nvram get wan0_dnsenable_x)" == "1" ]] >/dev/null;then
    if [ ! -z "$(echo $(nvram get wan0_dns) | awk '{print $1}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get wan0_dns) | awk '{print $1}') lookup "$WAN0" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get wan0_dns) | awk '{print $1}') lookup "$WAN0""
        ip rule add from $(echo $(nvram get wan0_dns) | awk '{print $1}') lookup "$WAN0" priority 200
      fi
    fi
    if [ ! -z "$(echo $(nvram get wan0_dns) | awk '{print $2}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get wan0_dns) | awk '{print $2}') lookup "$WAN0" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get wan0_dns) | awk '{print $2}') lookup "$WAN0""
        ip rule add from $(echo $(nvram get wan0_dns) | awk '{print $2}') lookup "$WAN0" priority 200
      fi
    fi
  fi
  # Create WAN1 IP Address Rule
  if [ -z "$(ip rule list from $(nvram get wan1_ipaddr) lookup "$WAN1" priority 200)" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get wan1_ipaddr) lookup "$WAN1""
    ip rule add from $(nvram get wan1_ipaddr) lookup "$WAN1" priority 200
  fi
  # Create WAN1 Gateway IP Rule
  if [ -z "$(ip rule list from all to $(nvram get wan1_gateway) lookup "$WAN1" priority 400)" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get wan1_gateway) lookup "$WAN1""
    ip rule add from all to $(nvram get wan1_gateway) lookup "$WAN1" priority 400
  fi
  # Create WAN1 DNS IP Rules
  if [[ "$(nvram get wan1_dnsenable_x)" == "0" ]] >/dev/null;then
    if [ ! -z "$(nvram get wan1_dns1_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get wan1_dns1_x) lookup "$WAN1" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get wan1_dns1_x) lookup "$WAN1""
        ip rule add from $(nvram get wan1_dns1_x) lookup "$WAN1" priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get wan1_dns1_x) lookup "$WAN1" priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get wan1_dns1_x) lookup "$WAN1""
        ip rule add from all to $(nvram get wan1_dns1_x) lookup "$WAN1" priority 400
      fi
   fi
   if [ ! -z "$(nvram get wan1_dns2_x)" ] >/dev/null;then
     if [ -z "$(ip rule list from $(nvram get wan1_dns2_x) lookup "$WAN1" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get wan1_dns2_x) lookup "$WAN1""
        ip rule add from $(nvram get wan1_dns2_x) lookup "$WAN1" priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get wan1_dns2_x) lookup "$WAN1" priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get wan1_dns2_x) lookup "$WAN1""
        ip rule add from all to $(nvram get wan1_dns2_x) lookup "$WAN1" priority 400
      fi
    fi
  elif [[ "$(nvram get wan1_dnsenable_x)" == "1" ]] >/dev/null;then
    if [ ! -z "$(echo $(nvram get wan1_dns) | awk '{print $1}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get wan1_dns) | awk '{print $1}') lookup "$WAN1" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get wan1_dns) | awk '{print $1}') lookup "$WAN1""
        ip rule add from $(echo $(nvram get wan1_dns) | awk '{print $1}') lookup "$WAN1" priority 200
      fi
    fi
    if [ ! -z "$(echo $(nvram get wan1_dns) | awk '{print $2}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get wan1_dns) | awk '{print $2}') lookup "$WAN1" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get wan1_dns) | awk '{print $2}') lookup "$WAN1""
        ip rule add from $(echo $(nvram get wan1_dns) | awk '{print $2}') lookup "$WAN1" priority 200
      fi
    fi
  fi
  ...
  if [[ "$OVPNSPLITTUNNEL" == "1" ]] >/dev/null;then
    ...
    for REMOTEADDRESS in ${REMOTEADDRESSES};do
      ...
      if [ -z "$(ip rule list from all to $REMOTEIP lookup "$WAN0" priority 100)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $REMOTEIP lookup "$WAN0""
        ip rule add from all to $REMOTEIP lookup "$WAN0" priority 100
      fi
      if [ ! -z "$(ip rule list from all to $REMOTEIP lookup "$WAN1" priority 100)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Removing IP Rule from all to $REMOTEIP lookup "$WAN1""
        ip rule del from all to $REMOTEIP lookup "$WAN1" priority 100
      fi
    done
  fi
fi
...
&& { [[ "$(nvram get wan0_gateway)" == "$(ip route list default table 100 | awk '{print $3}')" ]] && [[ "$(nvram get wan0_gw_ifname)" == "$(ip route list default table 100 | awk '{print $5}')" ]] ;} \
&& { [[ "$(nvram get wan1_gateway)" == "$(ip route list default table 200 | awk '{print $3}')" ]] && [[ "$(nvram get wan1_gw_ifname)" == "$(ip route list default table 200 | awk '{print $5}')" ]] ;} \
|| { [[ "$(nvram get wan0_state_t)" != "2" ]] || [[ "$(nvram get wan1_state_t)" != "2" ]] ;} >/dev/null;do
  ...
  if [ -z "$(ip rule list from all iif lo to "$WAN0TARGET" lookup "$WAN0")" ] && [[ "$(nvram get wan0_state_t)" == "2" ]] \
  || [ -z "$(ip rule list from all iif lo to "$WAN1TARGET" lookup "$WAN1")" ] && [[ "$(nvram get wan1_state_t)" == "2" ]] >/dev/null;then
    break
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      if [[ "$OVPNSPLITTUNNEL" == "0" ]] >/dev/null;then
        ...
        for REMOTEADDRESS in ${REMOTEADDRESSES};do
          REMOTEIP=$(nslookup $REMOTEADDRESS | awk '(NR>2) && /^Address/ {print $3}' | awk '!/:/')
          if [ -z "$(ip rule list from all to $REMOTEIP lookup "$WAN0" priority 100)" ] >/dev/null;then
            logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $REMOTEIP lookup "$WAN0""
            ip rule add from all to $REMOTEIP lookup "$WAN0" priority 100
          fi
          if [ ! -z "$(ip rule list from all to $REMOTEIP lookup "$WAN1" priority 100)" ] >/dev/null;then
            logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule from all to $REMOTEIP lookup "$WAN1""
            ip rule del from all to $REMOTEIP lookup "$WAN1" priority 100
          fi
        done
      fi
    ...
    if [ -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      # Create WAN1 IP Address Rule
      if [ -z "$(ip rule list from $(nvram get wan1_ipaddr) lookup "$WAN1" priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get wan1_ipaddr) lookup "$WAN1""
        ip rule add from $(nvram get wan1_ipaddr) lookup "$WAN1" priority 200
      fi

      # Create WAN1 Gateway IP Rule
      if [ -z "$(ip rule list from all to $(nvram get wan1_gateway) lookup "$WAN1" priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get wan1_gateway) lookup "$WAN1""
        ip rule add from all to $(nvram get wan1_gateway) lookup "$WAN1" priority 400
      fi
      ...
      if [[ "$OVPNSPLITTUNNEL" == "0" ]] >/dev/null;then
        ...
        for REMOTEADDRESS in ${REMOTEADDRESSES};do
          REMOTEIP=$(nslookup $REMOTEADDRESS | awk '(NR>2) && /^Address/ {print $3}' | awk '!/:/')
          if [ -z "$(ip rule list from all to $REMOTEIP lookup "$WAN1" priority 100)" ] >/dev/null;then
            logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $REMOTEIP lookup "$WAN1""
            ip rule add from all to $REMOTEIP lookup "$WAN1" priority 100
          fi
          if [ ! -z "$(ip rule list from all to $REMOTEIP lookup "$WAN0" priority 100)" ] >/dev/null;then
            logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule from all to $REMOTEIP lookup "$WAN0""
            ip rule del from all to $REMOTEIP lookup "$WAN0" priority 100
          fi
        done
      fi
      continue
...
&& [ ! -z "$(ip rule list from all iif lo to "$WAN0TARGET" lookup "$WAN0")" ] \
&& { [[ "$(nvram get wan0_gateway)" == "$(ip route list default table 100 | awk '{print $3}')" ]] && [[ "$(nvram get wan0_gw_ifname)" == "$(ip route list default table 100 | awk '{print $5}')" ]] ;} >/dev/null;do
  WAN0PACKETLOSS="$(ping -I $(nvram get wan0_gw_ifname) $WAN0TARGET -c $PINGCOUNT -W $PINGTIMEOUT | grep -e "packet loss" | awk '{print $7}')"
  if [[ "$WAN0PACKETLOSS" == "0%" ]] >/dev/null;then
    continue
...
while \
  # Check Version of IP
  if [[ "$(ip -V | awk -F "iproute2-" '{print $2}')" != "5.11.0" ]] >/dev/null;then
