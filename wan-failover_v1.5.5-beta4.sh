  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    # Check if WAN is Enabled
      ...
      if [[ "${WANPREFIX}" == "$(echo $WANPREFIXES | awk '{print $1}')" ]] >/dev/null;then
        ...
        elif [ -z "$(ip route list default table 100 | grep -e "$(nvram get ${WANPREFIX}_gw_ifname)")" ] >/dev/null;then
        ...
        if [ -z "$(ip rule list from all to $WAN0TARGET iif lo lookup ${WANPREFIX})" ] >/dev/null;then
          ...
          ip rule add from all iif lo to $WAN0TARGET table 100 priority 100
      ...
      # Check WAN1 Connection
        ...
        # Check WAN1 IP Address Target Route
          ...
          logger -t "${0##*/}" "WAN Status - Default route already exists via "$(nvram get ${WANPREFIX}_gateway)" dev "$(nvram get ${WANPREFIX}_gw_ifname)""
        elif [ -z "$(ip route list default table 200 | grep -e "$(nvram get ${WANPREFIX}_gw_ifname)")" ] >/dev/null;then
          logger -t "${0##*/}" "WAN Status - Adding default route for ${WANPREFIX} Routing Table via "$(nvram get ${WANPREFIX}_gateway)" dev "$(nvram get ${WANPREFIX}_gw_ifname)""
        ...
        if [ -z "$(ip rule list from all to $WAN1TARGET iif lo lookup ${WANPREFIX})" ] >/dev/null;then
          ...
          ip rule add from all iif lo to $WAN1TARGET table 200 priority 100
...
if [[ "$(nvram get wans_mode)" == "lb" ]] >/dev/null;then
  ...
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /balance/ && /'$(nvram get lan_ifname)'/ && /state/ && /NEW/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IPTables MANGLE Balance Rule"
    iptables -t mangle -A PREROUTING -i $(nvram get lan_ifname) -m state --state NEW -j balance
  fi
  # Check IPTables Mangle Match Rule for WAN0 for PREROUTING Table
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get lan_ifname)'/ && /connmark match/ && /0x80000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IPTables MANGLE match rule for $(nvram get lan_ifname) marked with 0x80000000"
    iptables -t mangle -A PREROUTING -i $(nvram get lan_ifname) -m connmark --mark 0x80000000/0x80000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  # Check IPTables Mangle Match Rule for WAN0 for OUTPUT Table
  if [ -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname)'/ && /connmark match/ && /0x80000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IPTables MANGLE match rule for $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname) marked with 0x80000000"
    iptables -t mangle -A OUTPUT -o $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname) -m connmark --mark 0x80000000/0x80000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  if [ ! -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname)'/ && /connmark match/ && /0x90000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Deleting IPTables MANGLE match rule for $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname) marked with 0x90000000"
    iptables -t mangle -D OUTPUT -o $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname) -m connmark --mark 0x90000000/0x90000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  # Check IPTables Mangle Match Rule for WAN1 for PREROUTING Table
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get lan_ifname)'/ && /connmark match/ && /0x90000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IPTables MANGLE match rule for $(nvram get lan_ifname) marked with 0x90000000"
    iptables -t mangle -A PREROUTING -i $(nvram get lan_ifname) -m connmark --mark 0x90000000/0x90000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  # Check IPTables Mangle Match Rule for WAN1 for OUTPUT Table
  if [ -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname)'/ && /connmark match/ && /0x90000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IPTables MANGLE match rule for $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname) marked with 0x90000000"
    iptables -t mangle -A OUTPUT -o $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname) -m connmark --mark 0x90000000/0x90000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  if [ ! -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname)'/ && /connmark match/ && /0x80000000/ && /CONNMARK/ && /restore/ && /mask/ && /0xf0000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Deleting IPTables MANGLE match rule for $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname) marked with 0x80000000"
    iptables -t mangle -D OUTPUT -o $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname) -m connmark --mark 0x80000000/0x80000000 -j CONNMARK --restore-mark --mask 0xf0000000
  fi
  # Check IPTables Mangle Set XMark Rule for WAN0 for PREROUTING Table
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname)'/ && /state/ && /NEW/ && /CONNMARK/ && /xset/ && /0x80000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IPTables MANGLE set xmark rule for $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname)"
    iptables -t mangle -A PREROUTING -i $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname) -m state --state NEW -j CONNMARK --set-xmark 0x80000000/0xf0000000
  fi
  # Check IPTables Mangle Set XMark Rule for WAN1 for PREROUTING Table
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname)'/ && /state/ && /NEW/ && /CONNMARK/ && /xset/ && /0x90000000/ ) print}')" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IPTables MANGLE set xmark rule for $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname)"
    iptables -t mangle -A PREROUTING -i $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname) -m state --state NEW -j CONNMARK --set-xmark 0x90000000/0xf0000000
  fi
  # Create WAN0 IP Address Rule
  if [ -z "$(ip rule list from $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_ipaddr) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200)" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_ipaddr) lookup $(echo $WANPREFIXES | awk '{print $1}')"
    ip rule add from $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_ipaddr) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200
  fi
  # Create WAN0 Gateway IP Rule
  if [ -z "$(ip rule list from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gateway) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 400)" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gateway) lookup $(echo $WANPREFIXES | awk '{print $1}')"
    ip rule add from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gateway) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 400
  fi
  # Create WAN0 DNS IP Rules
  if [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dnsenable_x)" == "0" ]] >/dev/null;then
    if [ ! -z "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns1_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $1}')"
        ip rule add from $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $1}')"
        ip rule add from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 400
      fi
    fi
    if [ ! -z "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns2_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $1}')"
        ip rule add from $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $1}')"
        ip rule add from all to $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $1}') priority 400
      fi
    fi
  elif [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dnsenable_x)" == "1" ]] >/dev/null;then
    if [ ! -z "$(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $1}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $1}') lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $1}') lookup $(echo $WANPREFIXES | awk '{print $1}')"
        ip rule add from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $1}') lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200
      fi
    fi
    if [ ! -z "$(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $2}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $2}') lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $2}') lookup $(echo $WANPREFIXES | awk '{print $1}')"
        ip rule add from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $1}')_dns) | awk '{print $2}') lookup $(echo $WANPREFIXES | awk '{print $1}') priority 200
      fi
    fi
  fi
  # Create WAN1 IP Address Rule
  if [ -z "$(ip rule list from $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_ipaddr) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200)" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_ipaddr) lookup $(echo $WANPREFIXES | awk '{print $2}')"
    ip rule add from $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_ipaddr) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200
  fi
  # Create WAN1 Gateway IP Rule
  if [ -z "$(ip rule list from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gateway) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 400)" ] >/dev/null;then
    logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gateway) lookup $(echo $WANPREFIXES | awk '{print $2}')"
    ip rule add from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gateway) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 400
  fi
  # Create WAN1 DNS IP Rules
  if [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dnsenable_x)" == "0" ]] >/dev/null;then
    if [ ! -z "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns1_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $2}')"
        ip rule add from $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $2}')"
        ip rule add from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns1_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 400
      fi
   fi
   if [ ! -z "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns2_x)" ] >/dev/null;then
     if [ -z "$(ip rule list from $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $2}')"
        ip rule add from $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $2}')"
        ip rule add from all to $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns2_x) lookup $(echo $WANPREFIXES | awk '{print $2}') priority 400
      fi
    fi
  elif [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dnsenable_x)" == "1" ]] >/dev/null;then
    if [ ! -z "$(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $1}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $1}') lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $1}') lookup $(echo $WANPREFIXES | awk '{print $2}')"
        ip rule add from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $1}') lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200
      fi
    fi
    if [ ! -z "$(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $2}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $2}') lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $2}') lookup $(echo $WANPREFIXES | awk '{print $2}')"
        ip rule add from $(echo $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_dns) | awk '{print $2}') lookup $(echo $WANPREFIXES | awk '{print $2}') priority 200
      fi
    fi
  fi

# Create Array for OVPN Configs
OVPNCONFIGFILES='
...
# Create IP Rules for OVPN Remote Addresses
  for REMOTEADDRESS in ${REMOTEADDRESSES};do
    ...
    if [ -z "$(ip rule list from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $1}') priority 100)" ] >/dev/null;then
      logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $1}')"
      ip rule add from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $1}') priority 100
    fi
    if [ ! -z "$(ip rule list from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $2}') priority 100)" ] >/dev/null;then
      logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $2}')"
      ip rule del from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $2}') priority 100
    fi
  done
fi
...
&& { [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gateway)" == "$(ip route list default table 100 | awk '{print $3}')" ]] && [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gw_ifname)" == "$(ip route list default table 100 | awk '{print $5}')" ]] ;} \
&& { [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gateway)" == "$(ip route list default table 200 | awk '{print $3}')" ]] && [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gw_ifname)" == "$(ip route list default table 200 | awk '{print $5}')" ]] ;} \
|| { [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_state_t)" != "2" ]] || [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_state_t)" != "2" ]] ;} >/dev/null;do
  ...
  if [ -z "$(ip rule list from all to "$WAN0TARGET" iif lo lookup "$(echo $WANPREFIXES | awk '{print $1}')")" ] && [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_state_t)" == "2" ]] \
  || [ -z "$(ip rule list from all to "$WAN1TARGET" iif lo lookup "$(echo $WANPREFIXES | awk '{print $2}')")" ] && [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_state_t)" == "2" ]] >/dev/null;then
    break
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      if [ -z "$(ip rule list from all fwmark 0x80000000/0xf0000000 lookup wan0 priority 150)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for fwmark 0x80000000/0xf0000000 lookup wan0"
        ip rule add from all fwmark 0x80000000/0xf0000000 lookup wan0 priority 150
      fi
      if [ ! -z "$(ip rule list from all fwmark 0x80000000/0xf0000000 | grep -w "blackhole")" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Removing Blackhole IP Rule for fwmark 0x80000000/0xf0000000"
        ip rule del blackhole from all fwmark 0x80000000/0xf0000000 priority 150
      fi
      if [ -z "$(ip rule list from all fwmark 0x90000000/0xf0000000 lookup wan1 priority 150)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for fwmark 0x90000000/0xf0000000 lookup wan1"
        ip rule add from all fwmark 0x90000000/0xf0000000 lookup wan1 priority 150
      fi
      if [ ! -z "$(ip rule list from all fwmark 0x90000000/0xf0000000 | grep -w "blackhole")" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Removing Blackhole IP Rule for fwmark 0x90000000/0xf0000000"
        ip rule del blackhole from all fwmark 0x90000000/0xf0000000 priority 150
      fi
...
# Create IP Rules for OVPN Remote Addresses
  ...
  elif { [[ "$WAN0PACKETLOSS" == "100%" ]] || [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_state_t)" != "2" ]] ;} && [[ "$WAN1PACKETLOSS" == "0%" ]] >/dev/null;then
    if [ -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      continue
    else
      ...
      # Create fwmark IP Rules
      if [ ! -z "$(ip rule list from all fwmark 0x80000000/0xf0000000 lookup wan0 priority 150)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule for fwmark 0x80000000/0xf0000000 lookup wan0"
        ip rule del from all fwmark 0x80000000/0xf0000000 lookup wan0 priority 150
      fi
      if [ -z "$(ip rule list from all fwmark 0x80000000/0xf0000000 | grep -w "blackhole")" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding Blackhole IP Rule for fwmark 0x80000000/0xf0000000"
        ip rule add blackhole from all fwmark 0x80000000/0xf0000000 priority 150
      fi
...
# Create IP Rules for OVPN Remote Addresses
  for REMOTEADDRESS in ${REMOTEADDRESSES};do
    REMOTEIP=$(nslookup $REMOTEADDRESS | awk '(NR>2) && /^Address/ {print $3}' | awk '!/:/')
    if [ -z "$(ip rule list from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $2}') priority 100)" ] >/dev/null;then
      logger -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $2}')"
      ip rule add from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $2}') priority 100
    fi
    if [ ! -z "$(ip rule list from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $1}') priority 100)" ] >/dev/null;then
      logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $1}')"
      ip rule del from all to $REMOTEIP lookup $(echo $WANPREFIXES | awk '{print $1}') priority 100
    fi
  done
      continue
    fi
  elif [[ "$WAN0PACKETLOSS" == "0%" ]] && { [[ "$WAN1PACKETLOSS" == "100%" ]] || [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_state_t)" != "2" ]] ;} >/dev/null;then
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      continue
    else
      logger -t "${0##*/}" "Load Balance Monitor - Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS WAN1 Packet Loss: $WAN1PACKETLOSS"
      ...
      # Create fwmark IP Rules
      if [ ! -z "$(ip rule list from all fwmark 0x90000000/0xf0000000 lookup wan1 priority 150)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule for fwmark 0x90000000/0xf0000000 lookup wan1"
        ip rule del from all fwmark 0x80000000/0xf0000000 lookup wan0 priority 150
      fi
      if [ -z "$(ip rule list from all fwmark 0x90000000/0xf0000000 | grep -w "blackhole")" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Adding Blackhole IP Rule for fwmark 0x90000000/0xf0000000"
        ip rule add blackhole from all fwmark 0x80000000/0xf0000000 priority 150
      fi
...
# Create IP Rules for OVPN Remote Addresses
  ...
  elif { [[ "$WAN0PACKETLOSS" == "100%" ]] || [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_state_t)" != "2" ]] ;} \
  && { [[ "$WAN1PACKETLOSS" == "100%" ]] || [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_state_t)" != "2" ]] ;} >/dev/null;then
    if [ -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      continue
    else
      logger -t "${0##*/}" "Load Balance Monitor - Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS WAN1 Packet Loss: $WAN1PACKETLOSS"
      ...
      logger -t "${0##*/}" "Load Balance Monitor - Removing nexthop via $(nvram get wan1_gateway) dev $(nvram get wan1_gw_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $2}')"
      ip route del default
      if [ ! -z "$(ip rule list from all fwmark 0x80000000/0xf0000000 lookup wan0)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule for fwmark 0x80000000/0xf0000000 lookup wan0"
        ip rule del from all fwmark 0x80000000/0xf0000000 lookup wan0 priority 150
      fi
      if [ -z "$(ip rule list from all fwmark 0x80000000/0xf0000000 | grep -w "blackhole")" ] >/dev/null;then
      ...
      if [ ! -z "$(ip rule list from all fwmark 0x90000000/0xf0000000 lookup wan1)" ] >/dev/null;then
        logger -t "${0##*/}" "Load Balance Monitor - Removing IP Rule for fwmark 0x90000000/0xf0000000 lookup wan1"
        ip rule del from all fwmark 0x80000000/0xf0000000 lookup wan0 priority 150
      fi
      if [ -z "$(ip rule list from all fwmark 0x90000000/0xf0000000 | grep -w "blackhole")" ] >/dev/null;then
