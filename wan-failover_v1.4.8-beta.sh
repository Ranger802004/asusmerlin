...
&& { [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_gateway)" == "$(ip route show $WAN0TARGET | awk '{print $3}')" ]] && [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $1}')_ifname)" == "$(ip route show $WAN0TARGET | awk '{print $5}')" ]] ;} \
&& { [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_gateway)" == "$(ip route show $WAN1TARGET | awk '{print $3}')" ]] && [[ "$(nvram get $(echo $WANPREFIXES | awk '{print $2}')_ifname)" == "$(ip route show $WAN1TARGET | awk '{print $5}')" ]] ;};do
  ...
  WAN1PACKETLOSS="$(ping -I $(nvram get $(echo $WANPREFIXES | awk '{print $2}')_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT | grep -e "packet loss" | awk '{print $7}')"
  if [ -z "$(ip route list $WAN0TARGET via "$(nvram get "$(echo $WANPREFIXES | awk '{print $1}')"_gateway)" dev "$(nvram get "$(echo $WANPREFIXES | awk '{print $1}')"_ifname)")" ] \
  || [ -z "$(ip route list $WAN1TARGET via "$(nvram get "$(echo $WANPREFIXES | awk '{print $2}')"_gateway)" dev "$(nvram get "$(echo $WANPREFIXES | awk '{print $2}')"_ifname)")" ] >/dev/null;then
    break
    echo "Break"
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -t "${0##*/}" "Load Balance Monitor - Adding nexthop via $(nvram get wan0_gateway) dev $(nvram get wan0_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $1}')"
      logger -t "${0##*/}" "Load Balance Monitor - Adding nexthop via $(nvram get wan1_gateway) dev $(nvram get wan1_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $2}')"
      ip route add default scope global \
      nexthop via $(nvram get wan0_gateway) dev $(nvram get wan0_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $1}') \
      nexthop via $(nvram get wan1_gateway) dev $(nvram get wan1_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $2}')
      continue
    fi
    ...
    if [ -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -t "${0##*/}" "Load Balance Monitor - Removing nexthop via $(nvram get wan0_gateway) dev $(nvram get wan0_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $1}')"
      ip route del default
      ip route add default scope global \
      nexthop via $(nvram get wan1_gateway) dev $(nvram get wan1_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $2}')
      continue
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -t "${0##*/}" "Load Balance Monitor - Failure Detected - WAN0 Packet Loss: $WAN0PACKETLOSS WAN1 Packet Loss: $WAN1PACKETLOSS"
      logger -t "${0##*/}" "Load Balance Monitor - Removing nexthop via $(nvram get wan1_gateway) dev $(nvram get wan1_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $2}')"
      ip route del default
      ip route add default scope global \
      nexthop via $(nvram get wan0_gateway) dev $(nvram get wan0_ifname) weight $(echo "$(nvram get wans_lb_ratio)" | awk -F ":" '{print $1}')
      continue
