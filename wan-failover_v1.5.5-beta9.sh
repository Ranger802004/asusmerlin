  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    if [[ "$(nvram get misc_http_x)" == "1" ]] >/dev/null;then
      ...
      if [ -z "$(iptables -t nat -L PREROUTING -v -n | awk '{ if( /VSERVER/ && /'$(nvram get ${WANPREFIX}_ipaddr)'/ ) print}' )" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - ${WANPREFIX} creating VSERVER Rule for $(nvram get ${WANPREFIX}_ipaddr)"
    ...
    if [[ "$(nvram get ${WANPREFIX}_upnp_enable)" == "1" ]] >/dev/null;then
      ...
      if [ -z "$(iptables -t nat -L POSTROUTING -v -n | awk '{ if( /PUPNP/ && /'$(nvram get ${WANPREFIX}_gw_ifname)'/ ) print}' )" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - ${WANPREFIX} creating UPNP Rule for $(nvram get ${WANPREFIX}_gw_ifname)"
    ...
    if [[ "$(nvram get ${WANPREFIX}_nat_x)" == "1" ]] >/dev/null;then
      ...
      if [ -z "$(iptables -t nat -L POSTROUTING -v -n | awk '{ if( /MASQUERADE/ && /'$(nvram get ${WANPREFIX}_gw_ifname)'/ && /'$(nvram get ${WANPREFIX}_ipaddr)'/ ) print}')" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding iptables MASQUERADE rule for excluding $(nvram get ${WANPREFIX}_ipaddr) via $(nvram get ${WANPREFIX}_gw_ifname)"
...
for WANPREFIX in ${WANPREFIXES};do
  ...
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get lan_ifname)'/ && /connmark match/ && /'$MARK'/ && /CONNMARK/ && /restore/ && /mask/ && /'$MASK'/ ) print}')" ] >/dev/null;then
    logger -p 5 -t "${0##*/}" "WAN Status - Adding IPTables MANGLE match rule for $(nvram get lan_ifname) marked with "$MARK""
  ...
  if [ -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get ${WANPREFIX}_gw_ifname)'/ && /connmark match/ && /'$MARK'/ && /CONNMARK/ && /restore/ && /mask/ && /'$MASK'/ ) print}')" ] >/dev/null;then
    logger -p 5 -t "${0##*/}" "WAN Status - Adding IPTables MANGLE match rule for $(nvram get ${WANPREFIX}_gw_ifname) marked with "$MARK""
  ...
  if [ ! -z "$(iptables -t mangle -L OUTPUT -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get ${WANPREFIX}_gw_ifname)'/ && /connmark match/ && /'$DELETEMARK'/ && /CONNMARK/ && /restore/ && /mask/ && /'$MASK'/ ) print}')" ] >/dev/null;then
    logger -p 6 -t "${0##*/}" "WAN Status - Deleting IPTables MANGLE match rule for $(nvram get ${WANPREFIX}_gw_ifname) marked with "$DELETEMARK""
  ...
  if [ -z "$(iptables -t mangle -L PREROUTING -v -n | awk '{ if( /CONNMARK/ && /'$(nvram get ${WANPREFIX}_gw_ifname)'/ && /state/ && /NEW/ && /CONNMARK/ && /xset/ && /'$MARK'/ ) print}')" ] >/dev/null;then
    logger -p 5 -t "${0##*/}" "WAN Status - Adding IPTables MANGLE set xmark rule for $(nvram get ${WANPREFIX}_gw_ifname)"
  ...
  if [[ "$(nvram get ${WANPREFIX}_ipaddr)" != "0.0.0.0" ]] && [ -z "$(ip rule list from $(nvram get ${WANPREFIX}_ipaddr) lookup ${TABLE} priority "$FROMWANPRIORITY")" ] >/dev/null;then
    logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get ${WANPREFIX}_ipaddr) lookup ${TABLE}"
  ...
  if [[ "$(nvram get ${WANPREFIX}_gateway)" != "0.0.0.0" ]] && [ -z "$(ip rule list from all to $(nvram get ${WANPREFIX}_gateway) lookup ${TABLE} priority "$TOWANPRIORITY")" ] >/dev/null;then
    logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get ${WANPREFIX}_gateway) lookup ${TABLE}"
  ...
  if [[ "$(nvram get ${WANPREFIX}_dnsenable_x)" == "0" ]] >/dev/null;then
    if [ ! -z "$(nvram get ${WANPREFIX}_dns1_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get ${WANPREFIX}_dns1_x) lookup ${TABLE} priority "$FROMWANPRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get ${WANPREFIX}_dns1_x) lookup ${TABLE}"
      ...
      if [ -z "$(ip rule list from all to $(nvram get ${WANPREFIX}_dns1_x) lookup ${TABLE} priority "$TOWANPRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get ${WANPREFIX}_dns1_x) lookup ${TABLE}"
    ...
    if [ ! -z "$(nvram get ${WANPREFIX}_dns2_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get ${WANPREFIX}_dns2_x) lookup ${TABLE} priority "$FROMWANPRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get ${WANPREFIX}_dns2_x) lookup ${TABLE}"
      ...
      if [ -z "$(ip rule list from all to $(nvram get ${WANPREFIX}_dns2_x) lookup ${TABLE} priority "$TOWANPRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get ${WANPREFIX}_dns2_x) lookup ${TABLE}"
    ...
    if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}') lookup ${TABLE} priority "$FROMWANPRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}') lookup ${TABLE}"
    ...
    if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}') lookup ${TABLE} priority "$FROMWANPRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}') lookup ${TABLE}"
  ...
  if [[ "$OVPNSPLITTUNNEL" == "1" ]] >/dev/null;then
    logger -p 5 -t "${0##*/}" "WAN Status - OpenVPN Split Tunneling is Enabled"
  elif [[ "$OVPNSPLITTUNNEL" == "0" ]] >/dev/null;then
    logger -p 5 -t "${0##*/}" "WAN Status - OpenVPN Split Tunneling is Disabled"
    ...
    for REMOTEADDRESS in ${REMOTEADDRESSES};do
      ...
      if [ -z "$(ip rule list from all to $REMOTEIP lookup "$WAN0ROUTETABLE" priority "$OVPNWAN0PRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule from all to $REMOTEIP lookup "$WAN0ROUTETABLE" priority "$OVPNWAN0PRIORITY""
      ...
      if [ -z "$(ip rule list from all to $REMOTEIP lookup "$WAN1ROUTETABLE" priority "$OVPNWAN1PRIORITY")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "WAN Status - Adding IP Rule from all to $REMOTEIP lookup "$WAN1ROUTETABLE" priority "$OVPNWAN1PRIORITY""
...
if [[ "$WAN0STATUS" == "CONNECTED" ]] >/dev/null;then
  logger -p 5 -st "${0##*/}" "Load Balance Monitor - Monitoring "$WAN0" via "$WAN0TARGET" for Packet Loss"
...
if [[ "$WAN1STATUS" == "CONNECTED" ]] >/dev/null;then
  logger -p 5 -st "${0##*/}" "Load Balance Monitor - Monitoring "$WAN1" via "$WAN1TARGET" for Packet Loss"
  ...
  || { [ -z "$(ip rule list from all iif lo to "$WAN1TARGET" lookup "$WAN1ROUTETABLE")" ] && [[ "$(nvram get wan1_state_t)" == "2" ]] ;} && { [[ "$(nvram get wan1_ipaddr)" != "0.0.0.0" ]] || [[ "$(nvram get wan1_gateway)" != "0.0.0.0" ]] ;} >/dev/null;then
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      if [ -z "$(ip rule list from all fwmark "$WAN0MARK"/"$WAN0MASK" lookup "$WAN0ROUTETABLE" priority "$LBRULEPRIORITY")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for fwmark "$WAN0MARK"/"$WAN0MASK" lookup "$WAN0ROUTETABLE""
      ...
      if [ ! -z "$(ip rule list from all fwmark "$WAN0MARK"/"$WAN0MASK" | grep -w "blackhole")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Removing Blackhole IP Rule for fwmark "$WAN0MARK"/"$WAN0MASK""
      ...
      if [ -z "$(ip rule list from all fwmark "$WAN1MARK"/"$WAN1MASK" lookup "$WAN1ROUTETABLE" priority "$LBRULEPRIORITY")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for fwmark "$WAN1MARK"/"$WAN1MASK" lookup "$WAN1ROUTETABLE""
      ...
      if [ ! -z "$(ip rule list from all fwmark "$WAN1MARK"/"$WAN1MASK" | grep -w "blackhole")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Removing Blackhole IP Rule for fwmark "$WAN1MARK"/"$WAN1MASK""
      ...
      if [[ "$OVPNSPLITTUNNEL" == "0" ]] >/dev/null;then
        ...
        for REMOTEADDRESS in ${REMOTEADDRESSES};do
          ...
          REMOTEIP=$(nslookup $REMOTEADDRESS | awk '(NR>2) && /^Address/ {print $3}' | awk '!/:/')
          if [ -z "$(ip rule list from all to $REMOTEIP lookup "$WAN0ROUTETABLE" priority "$OVPNWAN0PRIORITY")" ] >/dev/null;then
            logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $REMOTEIP lookup "$WAN0ROUTETABLE" priority "$OVPNWAN0PRIORITY""
            ip rule add from all to $REMOTEIP lookup "$WAN0ROUTETABLE" priority "$OVPNWAN0PRIORITY"
          fi
          if [ -z "$(ip rule list from all to $REMOTEIP lookup "$WAN1ROUTETABLE" priority "$OVPNWAN1PRIORITY")" ] >/dev/null;then
            logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $REMOTEIP lookup "$WAN1ROUTETABLE" priority "$OVPNWAN1PRIORITY""
            ip rule add from all to $REMOTEIP lookup "$WAN1ROUTETABLE" priority "$OVPNWAN1PRIORITY"
          fi
        done
      fi
    ...
    if [ -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ ! -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -p 6 -t "${0##*/}" "Debug - Checking fwmark IP Rules"
      if [ ! -z "$(ip rule list from all fwmark "$WAN0MARK"/"$WAN0MASK" lookup "$WAN0ROUTETABLE" priority "$LBRULEPRIORITY")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Removing IP Rule for fwmark "$WAN0MARK"/"$WAN0MASK" lookup "$WAN0ROUTETABLE""
        ip rule del from all fwmark "$WAN0MARK"/"$WAN0MASK" lookup "$WAN0ROUTETABLE" priority "$LBRULEPRIORITY"
      fi
      if [ -z "$(ip rule list from all fwmark "$WAN0MARK"/"$WAN0MASK" | grep -w "blackhole")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding Blackhole IP Rule for fwmark "$WAN0MARK"/"$WAN0MASK""
        ip rule add blackhole from all fwmark "$WAN0MARK"/"$WAN0MASK" priority "$LBRULEPRIORITY"
      fi

      # Create WAN1 IP Address Rule
      logger -p 6 -t "${0##*/}" "Debug - Checking IP Address and Gateway IP Rules"
      if [ -z "$(ip rule list from $(nvram get wan1_ipaddr) lookup "$WAN1ROUTETABLE" priority "$FROMWAN1PRIORITY")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding IP Rule for $(nvram get wan1_ipaddr) lookup "$WAN1ROUTETABLE""
      ...
      if [ -z "$(ip rule list from all to $(nvram get wan1_gateway) lookup "$WAN1ROUTETABLE" priority "$TOWANPRIORITY")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding IP Rule from all to $(nvram get wan1_gateway) lookup "$WAN1ROUTETABLE""
      ...
      if [[ "$OVPNSPLITTUNNEL" == "0" ]] >/dev/null;then
        ...
        for REMOTEADDRESS in ${REMOTEADDRESSES};do
          ...
          if [ ! -z "$(ip rule list from all to $REMOTEIP lookup "$WAN0ROUTETABLE" priority "$OVPNWAN0PRIORITY")" ] >/dev/null;then
            logger -p 4 -t "${0##*/}" "Load Balance Monitor - Removing IP Rule from all to $REMOTEIP lookup "$WAN0ROUTETABLE" priority "$OVPNWAN0PRIORITY""
    ...
    if [ ! -z "$(ip route show default | grep -w "$(nvram get wan0_gateway)")" ] && [ -z "$(ip route show default | grep -w "$(nvram get wan1_gateway)")" ] >/dev/null;then
      ...
      logger -p 6 -t "${0##*/}" "Debug - Checking fwmark IP Rules"
      if [ ! -z "$(ip rule list from all fwmark "$WAN1MARK"/"$WAN1MASK" lookup "$WAN1ROUTETABLE" priority "$LBRULEPRIORITY")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Removing IP Rule for fwmark "$WAN1MARK"/"$WAN1MASK" lookup "$WAN1ROUTETABLE""
        ip rule del from all fwmark "$WAN1MARK"/"$WAN1MASK" lookup "$WAN1ROUTETABLE" priority "$LBRULEPRIORITY"
      fi
      if [ -z "$(ip rule list from all fwmark "$WAN1MARK"/"$WAN1MASK" | grep -w "blackhole")" ] >/dev/null;then
        logger -p 4 -t "${0##*/}" "Load Balance Monitor - Adding Blackhole IP Rule for fwmark "$WAN1MARK"/"$WAN1MASK""
      ...
      if [[ "$OVPNSPLITTUNNEL" == "0" ]] >/dev/null;then
        ...
        for REMOTEADDRESS in ${REMOTEADDRESSES};do
          ...
          if [ ! -z "$(ip rule list from all to $REMOTEIP lookup "$WAN1ROUTETABLE" priority "$OVPNWAN1PRIORITY")" ] >/dev/null;then
            logger -p 4 -t "${0##*/}" "Load Balance Monitor - Removing IP Rule from all to $REMOTEIP lookup "$WAN1ROUTETABLE" priority "$OVPNWAN1PRIORITY""
...
logger -p 5 -st "${0##*/}" "WAN0 Failover Monitor - Monitoring "$WAN0" via $WAN0TARGET for Failure"
...
if [[ "$(awk -F "." '{print $1}' "/proc/uptime")" -ge "$(($SKIPEMAILSYSTEMUPTIME+$BOOTDELAYTIMER))" ]] && { [ -f "$AIPROTECTION_EMAILCONFIG" ] || [ -f "$AMTM_EMAILCONFIG" ] ;} >/dev/null;then
  ...
  if [ -f "$AMTM_EMAILCONFIG" ] >/dev/null;then
    logger -p 5 -t "${0##*/}" "Email Notification - AMTM Email Configuration Detected"
    ...
    logger -p 5 -t "${0##*/}" "Email Notification - AIProtection Alerts Email Configuration Detected"
  ...
  logger -p 5 -t "${0##*/}" "Email Notification - Email Notifications are not configured"
fi
