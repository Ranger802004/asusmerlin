...
# Set Script Mode
if [ "$#" == "0" ] >/dev/null;then
  echo -e ""${BOLD}"${RED}${0##*/} - Executed without a Run Mode Selected!!!${NOCOLOR}"
  echo -e ""${BOLD}"${WHITE}Use one of the following run modes...${NOCOLOR}"
  echo -e ""${BOLD}"${BLUE}$0 install${WHITE} - Install the script and configuration files necessary.${NOCOLOR}"
  echo -e ""${BOLD}"${GREEN}$0 run${WHITE} - Execute the script in the background via Cron Job.${NOCOLOR}"
  echo -e ""${BOLD}"${GREEN}$0 manual${WHITE} - Execute the script in a command console.${NOCOLOR}"
  echo -e ""${BOLD}"${GREEN}$0 monitor${WHITE} - Monitor the System Log file for the script.${NOCOLOR}"
  echo -e ""${BOLD}"${YELLOW}$0 update${WHITE} - Download and update to the latest version.${NOCOLOR}"
  echo -e ""${BOLD}"${YELLOW}$0 config${WHITE} - Reconfiguration of WAN Failover to update or change settings.${NOCOLOR}"
  echo -e ""${BOLD}"${YELLOW}$0 cron${WHITE} - Create or delete the Cron Job necessary for the script to run.${NOCOLOR}"
  echo -e ""${BOLD}"${YELLOW}$0 switchwan${WHITE} - Manually switch Primary WAN.  ${RED}***Failover Mode Only***${NOCOLOR}"
  echo -e ""${BOLD}"${YELLOW}$0 email${WHITE} - Enable or disable email notifications using enable or disable parameter.${NOCOLOR}"
  echo -e ""${BOLD}"${RED}$0 uninstall${WHITE} - Uninstall the configuration files necessary to stop the script from running.${NOCOLOR}"
  echo -e ""${BOLD}"${RED}$0 restart${WHITE} - Restart the script if it is currently running.${NOCOLOR}"
  echo -e ""${BOLD}"${RED}$0 kill${WHITE} - Kill any instances of the script and unschedule Cron Jobs.${NOCOLOR}"
  break && exit
fi
mode="${1#}"
if [ "$#" == "2" ] >/dev/null;then
...
if [[ "${mode}" == "restart" ]] || [[ "${mode}" == "update" ]] || [[ "${mode}" == "config" ]] || [[ "$[mode}" == "email" ]] >/dev/null;then
  ...
  if [ ! -z "$PIDS" ] >/dev/null;then
    ...
    if tty >/dev/null 2>&1;then
      printf '\033[K%b\r' ""${BOLD}""${GREEN}"Successfully Restarted ${0##*/} Process ID(s): $PIDS "${NOCOLOR}""
      sleep 10
      printf '\033[K'
    fi
  elif [ -z "$PIDS" ] >/dev/null;then
...
if [ -z "$(awk -F "=" '/INTERFACE6IN4/ {print $1}' "$CONFIGFILE")" ] >/dev/null;then
  logger -p 6 -t "${0##*/}" "Debug - Setting INTERFACE6IN4 Default: wan0"
...
# Check if WAN Interfaces are Enabled and Connected
  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    # Check if WAN is Enabled
      ...
      if [[ "$(nvram get ${WANPREFIX}_primary)" == "1" ]] && [ ! -z "$(ip route list default table main | grep -e "$TARGET")" ] && { [[ "$(nvram get wans_mode)" == "fo" ]] || [[ "$(nvram get wans_mode)" == "fb" ]] ;} >/dev/null;then
      ...
      # Check WAN IP Rule
      if [ -z "$(ip rule list from all iif lo to $TARGET oif "$(nvram get ${WANPREFIX}_gw_ifname)" lookup ${TABLE} priority "$PRIORITY")" ] >/dev/null;then
...
for WANPREFIX in ${WANPREFIXES};do
  ...
  # Create IPv6 6in4 IP Rule
  if [[ "$(nvram get ipv6_service)" == "6in4" ]] >/dev/null;then
    logger -p 6 -t "${0##*/}" "Debug - IPv6 6in4 Interface: "$INTERFACE6IN4""
    logger -p 6 -t "${0##*/}" "Debug - IPv6 6in4 Tunnel IP Address: "$(nvram get ipv6_tun_v4end)""
    if [[ "${WANPREFIX}" == "$INTERFACE6IN4" ]] >/dev/null;then
      if [ -z "$(ip rule list from all iif lo to "$(nvram get ipv6_tun_v4end)" lookup ${TABLE} priority "$RULEPRIORITY6IN4")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "Check IP Rules - Adding IP Rule for "$(nvram get ipv6_tun_v4end)" lookup ${TABLE} priority "$RULEPRIORITY6IN4""
        ip rule add from all iif lo to $(nvram get ipv6_tun4end) lookup ${TABLE} priority "$RULEPRIORITY6IN4"
      fi
    elif [[ "${WANPREFIX}" != "$INTERFACE6IN4" ]] >/dev/null;then
      if [ ! -z "$(ip rule list from all iif lo to "$(nvram get ipv6_tun_v4end)" lookup ${TABLE} priority "$RULEPRIORITY6IN4")" ] >/dev/null;then
        logger -p 5 -t "${0##*/}" "Check IP Rules - Deleting IP Rule for "$(nvram get ipv6_tun_v4end)" lookup ${TABLE} priority "$RULEPRIORITY6IN4""
        ip rule del from all iif lo to $(nvram get ipv6_tun4end) lookup ${TABLE} priority "$RULEPRIORITY6IN4"
      fi
    fi
  fi
...
while [ "$i" -le "$RECURSIVEPINGCHECK" ] >/dev/null;do
  ...
  if [[ "$WAN0PACKETLOSS" == "0%" ]] && [[ "$WAN1PACKETLOSS" == "0%" ]] >/dev/null;then
    ...
    if tty >/dev/null 2>&1;then
      printf '\033[K%b\r\a' ""${BOLD}"$(date "+%D @ %T") - WAN0 Target: "${BLUE}""$WAN0TARGET" "${WHITE}"Packet Loss: "${RED}""$WAN0PACKETLOSS""${WHITE}" WAN1 Target: "${BLUE}""$WAN1TARGET" "${WHITE}"Packet Loss: "${GREEN}""$WAN1PACKETLOSS""${NOCOLOR}""
    fi
    i=$(($i+1))
    continue
    ...
    if tty >/dev/null 2>&1;then
      printf '\033[K%b\r\a' ""${BOLD}"$(date "+%D @ %T") - WAN0 Target: "${BLUE}""$WAN0TARGET" "${WHITE}"Packet Loss: "${GREEN}""$WAN0PACKETLOSS""${WHITE}" WAN1 Target: "${BLUE}""$WAN1TARGET" "${WHITE}"Packet Loss: "${RED}""$WAN1PACKETLOSS""${NOCOLOR}""
    fi
    i=$(($i+1))
    continue
    ...
    if tty >/dev/null 2>&1;then
      printf '\033[K%b\r\a' ""${BOLD}"$(date "+%D @ %T") - WAN0 Target: "${BLUE}""$WAN0TARGET" "${WHITE}"Packet Loss: "${RED}""$WAN0PACKETLOSS""${WHITE}" WAN1 Target: "${BLUE}""$WAN1TARGET" "${WHITE}"Packet Loss: "${RED}""$WAN1PACKETLOSS""${NOCOLOR}""
    fi
    i=$(($i+1))
    continue
...
logger -p 5 -t "${0##*/}" "WAN0 Failover Monitor - Verifying WAN0"
...
while { [[ "$(nvram get wan0_enable)" == "1" ]] && [[ "$(nvram get wan1_enable)" == "1" ]] ;} && [[ "$(nvram get wan0_primary)" == "1" ]] >/dev/null;do
  pingtargets || wanstatus
  if { [ -z "$(ip rule list from all iif lo to "$WAN0TARGET" oif "$(nvram get wan0_gw_ifname)" lookup "$WAN0ROUTETABLE")" ] \
  || { [[ "$(nvram get wan0_gateway)" != "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $3}')" ]] || [[ "$(nvram get wan0_gw_ifname)" != "$(ip route list default table "$WAN0ROUTETABLE" | awk '{print $5}')" ]] ;} ;} \
  && { [ -z "$(ip rule list from all iif lo to "$WAN1TARGET" oif "$(nvram get wan1_gw_ifname)" lookup "$WAN1ROUTETABLE")" ] \
  || { [[ "$(nvram get wan1_gateway)" != "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $3}')" ]] || [[ "$(nvram get wan1_gw_ifname)" != "$(ip route list default table "$WAN1ROUTETABLE" | awk '{print $5}')" ]] ;} ;} >/dev/null;then
    break
  elif [[ "$WAN0PACKETLOSS" == "0%" ]] && [[ "$WAN1PACKETLOSS" == "0%" ]] >/dev/null;then
    continue
  elif [[ "$WAN0PACKETLOSS" == "100%" ]] && [[ "$WAN1PACKETLOSS" == "100%" ]] >/dev/null;then
    ...
    WAN1STATUS=DISCONNECTED
    wandisabled
  elif [[ "$WAN0PACKETLOSS" == "100%" ]] \
  && { [[ "$WAN1PACKETLOSS" == "0%" ]] || [[ "$WAN1PACKETLOSS" != "0%" ]] ;} >/dev/null;then
    WAN0STATUS=DISCONNECTED
    WAN1STATUS=CONNECTED
    switchwan
  elif [[ "$WAN0PACKETLOSS" == "0%" ]] && [[ "$WAN1PACKETLOSS" == "100%" ]] >/dev/null;then
    ...
    WAN1STATUS=DISCONNECTED
    wandisabled
  elif { [[ "$WAN0PACKETLOSS" != "0%" ]] && [[ "$WAN1PACKETLOSS" != "0%" ]] ;} \
  || { [[ "$WAN0PACKETLOSS" != "0%" ]] || [[ "$WAN1PACKETLOSS" != "0%" ]] ;} >/dev/null;then
    continue
  fi
done
...
while { [[ "$(nvram get wan0_enable)" == "1" ]] && [[ "$(nvram get wan1_enable)" == "1" ]] ;} && [[ "$(nvram get wan1_primary)" == "1" ]] >/dev/null;do
  ...
  && [[ "$WAN1PACKETLOSS" == "100%" ]] >/dev/null;then
    WAN0STATUS=CONNECTED
    WAN1STATUS=DISCONNECTED
    switchwan
  elif { [[ "$WAN0PACKETLOSS" != "0%" ]] && [[ "$WAN1PACKETLOSS" != "0%" ]] ;} \
...
if [[ "$(nvram get wans_dualwan | awk '{print $2}')" == "none" ]] >/dev/null;then
  logger -p 2 -st "${0##*/}" "WAN Failover Disabled - Dual WAN is not configured"
fi
if [[ "$(nvram get wandog_enable)" != "0" ]] >/dev/null;then
...
if { [[ "$(nvram get wan0_ipaddr)" == "0.0.0.0" ]] || [[ "$(nvram get wan0_gateway)" == "0.0.0.0" ]] ;} >/dev/null;then
...
if { [[ "$(nvram get wan1_ipaddr)" == "0.0.0.0" ]] || [[ "$(nvram get wan1_gateway)" == "0.0.0.0" ]] ;} >/dev/null;then
...
while \
  ...
  elif { [[ "$(ping -I $(nvram get wan0_gw_ifname) $WAN0TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | grep -e "packet loss" | awk '{print $7}')" == "0%" ]] \
  && [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | grep -e "packet loss" | awk '{print $7}')" == "0%" ]] ;} >/dev/null;then
  ...
  && { [[ "$(nvram get wan1_primary)" == "1" ]] && [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | grep -e "packet loss" | awk '{print $7}')" == "100%" ]] ;} >/dev/null;then
  ...
  elif  [[ "$(ping -I $(nvram get wan1_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | grep -e "packet loss" | awk '{print $7}')" == "0%" ]] \
  && { [[ "$(nvram get wan0_primary)" == "1" ]] && [[ "$(ping -I $(nvram get wan0_gw_ifname) $WAN1TARGET -c $PINGCOUNT -W $PINGTIMEOUT -s $PACKETSIZE | grep -e "packet loss" | awk '{print $7}')" == "100%" ]] ;} >/dev/null;then
    logger -p 3 -st "${0##*/}" "WAN Failover Disabled - "$WAN0" has 0% packet loss but is not Primary WAN"
    switchwan && break
...
for WANPREFIX in ${WANPREFIXES};do
  ...
  # Check DNS if Status is Connected or Primary WAN
    ...
    if [[ "$(nvram get ${WANPREFIX}_dnsenable_x)" == "0" ]] >/dev/null;then
      ...
      if [ ! -z "$(nvram get ${WANPREFIX}_dns1_x)" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns1_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding DNS1 Server: "$(nvram get ${WANPREFIX}_dns1_x)""
        if { [[ "$(nvram get wans_mode)" == "fo" ]] || [[ "$(nvram get wans_mode)" == "fb" ]] ;} >/dev/null;then
      ...
      if [ ! -z "$(nvram get ${WANPREFIX}_dns2_x)" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns2_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding DNS2 Server: "$(nvram get ${WANPREFIX}_dns2_x)""
        if { [[ "$(nvram get wans_mode)" == "fo" ]] || [[ "$(nvram get wans_mode)" == "fb" ]] ;} >/dev/null;then
    ...
    # Change Automatic ISP DNS Settings
      ...
      logger -p 4 -st "${0##*/}" "DNS Switch - Automatic DNS Settings from ISP: "$(nvram get ${WANPREFIX}_dns)""
      ...
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding DNS1 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')""
      ...
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')" ] && [ -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Adding DNS2 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')""
  ...
  # Check DNS if Status is Disconnected or not Primary WAN
    ...
    if [[ "$(nvram get ${WANPREFIX}_dnsenable_x)" == "0" ]] >/dev/null;then
      # Change Manual DNS1 Server
      if [ ! -z "$(nvram get ${WANPREFIX}_dns1_x)" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns1_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing DNS1 Server: "$(nvram get ${WANPREFIX}_dns1_x)""
        sed -i '/nameserver '$(nvram get ${WANPREFIX}_dns1_x)'/d' $DNSRESOLVFILE
      fi
      # Change Manual DNS2 Server
      if [ ! -z "$(nvram get ${WANPREFIX}_dns2_x)" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns2_x))")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing DNS2 Server: "$(nvram get ${WANPREFIX}_dns2_x)""
    ...
    # Remove Automatic ISP DNS Settings
      ...
      # Change Automatic DNS1 Server
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing DNS1 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')""
        sed -i '/nameserver '$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')'/d' $DNSRESOLVFILE
      fi
      # Change Automatic DNS2 Server
      if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')" ] && [ ! -z "$(awk -F " " '{print $2}' "$DNSRESOLVFILE" | grep -w "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')")" ] >/dev/null;then
        logger -p 4 -st "${0##*/}" "DNS Switch - Removing DNS2 Server: "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')""
...
if [ ! -z "$(cru l | grep -w "YazFi")" ] && [ -f "/jffs/scripts/YazFi" ] >/dev/null;then
  ...
  sh /jffs/scripts/YazFi check || continue
fi

if [[ "$(nvram get wans_mode)" == "lb" ]] >/dev/null;then
  return
elif [[ "${mode}" == "switchwan" ]] && { [[ "$(nvram get wans_mode)" == "fo" ]] || [[ "$(nvram get wans_mode)" == "fb" ]] ;} >/dev/null;then
  exit
...
if [[ "$(awk -F "." '{print $1}' "/proc/uptime")" -ge "$(($SKIPEMAILSYSTEMUPTIME+$BOOTDELAYTIMER))" ]] && { [ -f "$AIPROTECTION_EMAILCONFIG" ] || [ -f "$AMTM_EMAILCONFIG" ] ;} >/dev/null;then
  ...
  if [[ "$(nvram get wans_mode)" == "lb" ]] >/dev/null;then
    ...
    echo "Active ISP: $(/usr/sbin/curl --connect-timeout $EMAILTIMEOUT --max-time $EMAILTIMEOUT ipinfo.io | grep -e "org" | awk '{print $3" "$4}' | cut -f 1 -d "," | cut -f 1 -d '"' || continue)" >>"$TMPEMAILFILE"
