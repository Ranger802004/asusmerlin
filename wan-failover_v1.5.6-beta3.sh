...
if [[ "${mode}" == "restart" ]] || [[ "${mode}" == "update" ]] || [[ "${mode}" == "config" ]] || [[ "$[mode}" == "email" ]] >/dev/null;then
  ...
  logger -p 5 -st "${0##*/}" "Restart - Waiting for ${0##*/} to restart from CronJob"
    ...
    if [ ! -z "$PIDS" ] >/dev/null;then
      ...
      if tty >/dev/null 2>&1;then
        ...
        if [[ "$TIMEOUTTIMER" -ge "60" ]] >/dev/null;then
          printf '\033[K%b\r' ""${BOLD}""${BLUE}"***Waiting for ${0##*/} to restart from CronJob*** Timeout: "${GREEN}""$TIMEOUTTIMER" Seconds"${NOCOLOR}""
        elif [[ "$TIMEOUTTIMER" -ge "30" ]] >/dev/null;then
          printf '\033[K%b\r' ""${BOLD}""${BLUE}"***Waiting for ${0##*/} to restart from CronJob*** Timeout: "${YELLOW}""$TIMEOUTTIMER" Seconds"${NOCOLOR}""
        elif [[ "$TIMEOUTTIMER" -ge "0" ]] >/dev/null;then
          printf '\033[K%b\r' ""${BOLD}""${BLUE}"***Waiting for ${0##*/} to restart from CronJob*** Timeout: "${RED}""$TIMEOUTTIMER" Seconds"${NOCOLOR}""
        fi
      fi
      sleep 1
    fi
...
if [[ "(nvram get ipv6_service)" != "disabled" ]] >/dev/null;then
  logger -p 6 -t "${0##*/}" "Debug - IPv6 Service: "$(nvram get ipv6_service)""
  SERVICE="wan6"
  SERVICES="${SERVICES} ${SERVICE}"
fi
