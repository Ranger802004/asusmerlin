...
# Check if WAN Interfaces are Enabled and Connected
  ...
  for WANPREFIX in ${WANPREFIXES};do
    ...
    # Check if WAN is Enabled
      ...
      if [ ! -z "$(ip route list default table main | grep -e "$TARGET")" ] && [[ "$(nvram get wans_mode)" == "fo" ]] >/dev/null;then
      ...
      # Check WAN IP Rule
      if [ -z "$(ip rule list from all iif lo to $TARGET lookup ${WANPREFIX} priority "$PRIORITY")" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for "$TARGET""
        ip rule add from all iif lo to $TARGET table ${WANPREFIX} priority "$PRIORITY"
...
for WANPREFIX in ${WANPREFIXES};do
  ...
  if [[ "${WANPREFIX}" == "$WAN0" ]] >/dev/null;then
    MARK="0x80000000"
    DELETEMARK="0x90000000"
    MASK="0xf0000000"
  elif [[ "${WANPREFIX}" == "$WAN1" ]] >/dev/null;then
    MARK="0x90000000"
    DELETEMARK="0x80000000"
    MASK="0xf0000000"
  fi
  ...
  if [[ "$(nvram get ${WANPREFIX}_ipaddr)" != "0.0.0.0" ]] && [ -z "$(ip rule list from $(nvram get ${WANPREFIX}_ipaddr) lookup ${WANPREFIX} priority 200)" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get ${WANPREFIX}_ipaddr) lookup ${WANPREFIX}"
    ip rule add from $(nvram get ${WANPREFIX}_ipaddr) lookup ${WANPREFIX} priority 200
  fi
  # Create WAN Gateway IP Rule
  if [[ "$(nvram get ${WANPREFIX}_gateway)" != "0.0.0.0" ]] && [ -z "$(ip rule list from all to $(nvram get ${WANPREFIX}_gateway) lookup ${WANPREFIX} priority 400)" ] >/dev/null;then
    logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get ${WANPREFIX}_gateway) lookup ${WANPREFIX}"
    ip rule add from all to $(nvram get ${WANPREFIX}_gateway) lookup ${WANPREFIX} priority 400
  fi
  # Create WAN DNS IP Rules
  if [[ "$(nvram get ${WANPREFIX}_dnsenable_x)" == "0" ]] >/dev/null;then
    if [ ! -z "$(nvram get ${WANPREFIX}_dns1_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get ${WANPREFIX}_dns1_x) lookup ${WANPREFIX} priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get ${WANPREFIX}_dns1_x) lookup ${WANPREFIX}"
        ip rule add from $(nvram get ${WANPREFIX}_dns1_x) lookup ${WANPREFIX} priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get ${WANPREFIX}_dns1_x) lookup ${WANPREFIX} priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get ${WANPREFIX}_dns1_x) lookup ${WANPREFIX}"
        ip rule add from all to $(nvram get ${WANPREFIX}_dns1_x) lookup ${WANPREFIX} priority 400
      fi
    fi
    if [ ! -z "$(nvram get ${WANPREFIX}_dns2_x)" ] >/dev/null;then
      if [ -z "$(ip rule list from $(nvram get ${WANPREFIX}_dns2_x) lookup ${WANPREFIX} priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(nvram get ${WANPREFIX}_dns2_x) lookup ${WANPREFIX}"
        ip rule add from $(nvram get ${WANPREFIX}_dns2_x) lookup ${WANPREFIX} priority 200
      fi
      if [ -z "$(ip rule list from all to $(nvram get ${WANPREFIX}_dns2_x) lookup ${WANPREFIX} priority 400)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule from all to $(nvram get ${WANPREFIX}_dns2_x) lookup ${WANPREFIX}"
        ip rule add from all to $(nvram get ${WANPREFIX}_dns2_x) lookup ${WANPREFIX} priority 400
      fi
    fi
  elif [[ "$(nvram get ${WANPREFIX}_dnsenable_x)" == "1" ]] >/dev/null;then
    if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}') lookup ${WANPREFIX} priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}') lookup ${WANPREFIX}"
        ip rule add from $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $1}') lookup ${WANPREFIX} priority 200
      fi
    fi
    if [ ! -z "$(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}')" ] >/dev/null;then
      if [ -z "$(ip rule list from $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}') lookup ${WANPREFIX} priority 200)" ] >/dev/null;then
        logger -t "${0##*/}" "WAN Status - Adding IP Rule for $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}') lookup ${WANPREFIX}"
        ip rule add from $(echo $(nvram get ${WANPREFIX}_dns) | awk '{print $2}') lookup ${WANPREFIX} priority 200
      fi
    fi
  fi
done
