...
for QUERYPOLICY in ${QUERYPOLICIES};do
  ...
  for DOMAIN in ${DOMAINS}; do
    echo -e "${YELLOW}Query Policy: $QUERYPOLICY Querying "$DOMAIN"...${NOCOLOR}"
    ...
    for IP in $(nslookup $DOMAIN | awk '(NR>2) && /^Address/ {print $3}' | sort); do
      ...
      && [ ! -z "$(echo $IP | grep -oE "\b^(((10|127)(\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3})|(((172\.(1[6-9]|2[0-9]|3[0-1]))|(192\.168))(\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){2}))$\b")" ] >/dev/null;then
        echo -e "${RED}Query Policy: Domain: "$DOMAIN" queried "$IP" ***Excluded because Private IPs are disabled for Policy: "$QUERYPOLICY"***${NOCOLOR}"
        if [[ "$(cat "$CONFIGFILE" | grep -w "$QUERYPOLICY" | awk -F"|" '{print $5}')" == "VERBOSELOGGING=1" ]] >/dev/null;then
          logger -t "${0##*/}" "Query Policy - Domain: "$DOMAIN" queried "$IP" ***Excluded because Private IPs are disabled for Policy: "$QUERYPOLICY"***"
        fi
      fi
    done
  done
