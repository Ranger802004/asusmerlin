...
if [[ "$GETWANMODE" == "1" ]] &>/dev/null;then
  ...
  while [ -z "${wansync+x}" ] &>/dev/null || [[ "$wansync" == "0" ]] &>/dev/null;do
    ...
    # IPADDR
    IPADDR="$(nvram get ${WANPREFIX}_ipaddr & nvramcheck)" && { { [ ! -z "$IPADDR" ] &>/dev/null || [[ "$STATE" != "2" ]] &>/dev/null || [[ "$ENABLE" == "0" ]] &>/dev/null ;} || { logger -p 6 -t "$ALIAS" "Debug - failed to set IPADDR for "${WANPREFIX}"" && unset IPADDR && continue ;} ;}

    # GATEWAY
    GATEWAY="$(nvram get ${WANPREFIX}_gateway & nvramcheck)" && { { [ ! -z "$GATEWAY" ] &>/dev/null || [[ "$STATE" != "2" ]] &>/dev/null || [[ "$ENABLE" == "0" ]] &>/dev/null ;} || { logger -p 6 -t "$ALIAS" "Debug - failed to set GATEWAY for "${WANPREFIX}"" && unset GATEWAY && continue ;} ;}
